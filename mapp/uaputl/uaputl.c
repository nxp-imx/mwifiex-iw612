...
/** Option for cmd */
struct option cmd_options[] = {{"help", 0, 0, 'h'}, {0, 0, 0, 0}};
 ...
 */
static int is_protocol_valid(int protocol)
{
	int ret = UAP_FAILURE;
	switch (protocol) {
 ...
 */
__attribute__((format(printf, 2, 3))) static void uap_printf(int level,
							     char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
 ...
 *  @return            	None
 */
static void hexdump(char *prompt, void *p, int len, char delim)
{
 ...
 *  $return         N/A
 */
static void print_sys_info_usage(void)
{
	printf("\nUsage : sys_info\n");
	return;
}
 ...
 */
static t_u8 parse_domain_name(char *domain_name)
{
 ...
 */
static int apcmd_sys_info(int argc, char *argv[])
{
 ...
 *  $return         N/A
 */
static void print_deepsleep_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_deepsleep(int argc, char *argv[])
{
	int opt;
...
static int apcmd_band_steering(int argc, char *argv[])
{
	int opt;
	...
	if (argc) {
		...
		for (i = 0; i < argc; i = i + 2) {
			...
			} else if (strcmp(argv[i], "max_btm_req_allowed") ==
				   0) {
...
/**
 *  @brief Configure Beacon Stuck Detect Mechanism Parameters
 *
 *  Usage: beacon_stuck_detect_count recovery_confirm_count
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         UAP_SUCCESS/UAP_FAILURE
...
static int apcmd_beacon_stuck_detect(int argc, char *argv[])
{
	int opt;
	...
	while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
		switch (opt) {
		default:
			printf("\nUsage: beacon_stuck_detect [beacon_stuck_detect_count] [recovery_confirm_count]\n");
			return UAP_SUCCESS;
		}
	}
	...
	param.subcmd = UAP_BEACON_STUCK_DETECT;

	/** Set Command */
	if (argc) {
		beacon_stuck_detect_count = atoi(argv[0]);
		if (beacon_stuck_detect_count > 60) {
			printf("ERR: Enter Parameter value between 0 to 60");
			return UAP_FAILURE;
		}

		recovery_confirm_count = atoi(argv[1]);
		if (recovery_confirm_count > 60) {
			printf("ERR: Enter Parameter value between 0 to 60");
			return UAP_FAILURE;
		}

		param.beacon_stuck_detect_count = beacon_stuck_detect_count;
		param.recovery_confirm_count = recovery_confirm_count;
	...
	if (argc) {
		printf("Set Successfull\n");
		printf("beacon_stuck_detect_count = %d \n",
		       param.beacon_stuck_detect_count);
		printf("recovery_confirm_count = %d\n",
 ...
 *  $return         N/A
 */
static void print_txdatapause_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int send_txpause_ioctl(tx_data_pause_para *txpause)
{
	struct ifreq ifr;
	t_s32 sockfd;
	/* Open socket */
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_txdatapause(int argc, char *argv[])
{
	int opt;
 ...
 *  @return         N/A
 */
static int apcmd_hostcmd(int argc, char *argv[])
{
	apcmdbuf *hdr;
	t_u8 *buffer = NULL;
	int ret = UAP_SUCCESS;
	...
	if (ret == UAP_SUCCESS) {
		...
		if (!hdr->result) {
			printf("UAPHOSTCMD: CmdCode=%#04x, Size=%#04x, SeqNum=%#04x, Result=%#04x\n",
			       hdr->cmd_code, hdr->size, hdr->seq_num,
			       hdr->result);
			hexdump_data(
				"payload", (void *)(buffer + APCMDHEADERLEN),
 ...
 *  $return         N/A
 */
static void print_cmd52rw_usage(void)
{
 ...
 *  @return         N/A
 */
static int apcmd_cmd52_readwrite(int argc, char *argv[])
{
	int opt;
	sdcmd52_para param;
	struct ifreq ifr;
	...
	printf("func=%d, reg=%d, data=0x%02X\n", param.cmd52_params[0],
	       param.cmd52_params[1], param.cmd52_params[2]);
	/* Close socket */
	close(sockfd);
	return UAP_SUCCESS;
}
 ...
 *  $return         N/A
 */
static void print_addbapara_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_addbapara(int argc, char *argv[])
{
	int opt;
	addba_param param;
	struct ifreq ifr;
 ...
 *  $return         N/A
 */
static void print_aggrpriotbl_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_aggrpriotbl(int argc, char *argv[])
{
	int opt;
 ...
 *  $return         N/A
 */
static void print_addba_reject_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_addbareject(int argc, char *argv[])
{
	int opt;
 ...
 *  $return         N/A
 */
static void print_ht_tx_usage(void)
{
	printf("\nUsage : httxcfg [<txcfg>]");
 ...
 */
static int apcmd_sys_cfg_ht_tx(int argc, char *argv[])
{
	int opt;
 ...
 *  $return         N/A
 */
static void print_vht_usage(void)
{
	...
	printf("\n         txrx : This parameter specifies the configuration of VHT"
	       "operation for TX or/and VHT capabilities.");
	...
	printf("\n                      0: Tx BW follows the BW (20/40 MHz) from"
	       "11N CFG");
	printf("\n                      1: Tx BW follows the BW (80/160/80+80 MHz)"
	       "from VHT Capabilities.");
 ...
 *  $return         N/A
 */
static void print_vht_response(struct eth_priv_vhtcfg *vhtcfg)
{
	/* GET operation */
	/* Band */
 ...
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int apcmd_sys_cfg_vht(int argc, char *argv[])
{
	int opt, i = 0;
	vht_cfg_para param;
 ...
 *  $return         N/A
 */
static void print_tx_bf_usage(void)
{
 ...
 *  $return         N/A
 */
static void print_tx_bf_response(tx_bf_cfg_para *param)
{
	int i;
	...
	case TRIGGER_SOUNDING_FOR_PEER:
		printf("PEER MAC = %02X:%02X:%02X:%02X:%02X:%02X, STATUS = %s\n",
		       ...
		       bf_sound->peer_mac[4], bf_sound->peer_mac[5],
		       bf_sound->status ? "Failure" : "Success");
		break;
	case SET_GET_BF_PERIODICITY:
		printf("PEER MAC = %02x:%02x:%02x:%02x:%02x:%02x, Interval (ms) = %d\n",
		       bf_periodicity->peer_mac[0], bf_periodicity->peer_mac[1],
		       bf_periodicity->peer_mac[2], bf_periodicity->peer_mac[3],
		       bf_periodicity->peer_mac[4], bf_periodicity->peer_mac[5],
		       bf_periodicity->interval);
		break;
	case TX_BF_FOR_PEER_ENBL:
		for (i = 0; i < (int)param->no_of_peers; i++) {
			...
			printf("Sounding Status : %s\n",
			       tx_bf_peer->sounding_enbl ? "ENABLED" :
							   "DISABLED");
			printf("FB Type : %d\n", tx_bf_peer->fb_type);
			tx_bf_peer++;
		}
	...
	case SET_SNR_THR_PEER:
		for (i = 0; i < (int)param->no_of_peers; i++) {
			printf("PEER MAC = %02x:%02x:%02x:%02x:%02x:%02x, SNR = %d\n",
			       ...
			       bf_snr->peer_mac[4], bf_snr->peer_mac[5],
			       bf_snr->snr);
			bf_snr++;
		}
		break;
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_sys_cfg_tx_bf(int argc, char *argv[])
{
	...
	case TRIGGER_SOUNDING_FOR_PEER:
		...
		if ((ret = mac2raw(argv[1], param.body.bf_sound[0].peer_mac)) !=
		    UAP_SUCCESS) {
			printf("ERR: %s Address\n",
			       ret == UAP_FAILURE ?
				       "Invalid MAC" :
				       ret == UAP_RET_MAC_BROADCAST ?
				       "Broadcast" :
				       "Multicast");
			return UAP_FAILURE;
		}
 ...
 *  $return         N/A
 */
static void print_hscfg_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_hscfg(int argc, char *argv[])
{
	int opt;
	int i = 0;
	ds_hs_cfg hscfg;
 ...
 *  $return         N/A
 */
static void print_hssetpara_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_hssetpara(int argc, char *argv[])
{
	int opt;
	int i = 0;
	ds_hs_cfg hscfg;
 ...
 *  @return        UAP_SUCCESS/UAP_FAILURE
 */
static int uap_ioctl_cac_timer_status(unsigned int *mode)
{
	struct ifreq ifr;
 ...
 *  @return        UAP_SUCCESS/UAP_FAILURE
 */
static int send_power_mode_ioctl(ps_mgmt *pm, int flag)
{
	struct ifreq ifr;
	t_s32 sockfd;
	t_u32 result = 0;
 ...
 *  $return         N/A
 */
static void print_pscfg_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_pscfg(int argc, char *argv[])
{
	int opt;
	ps_mgmt pm;
	int ret = UAP_SUCCESS;
	...
	if (argc) {
		...
		if (argc >= 5) {
			...
			if (pm.sleep_param.min_sleep >
			    pm.sleep_param.max_sleep) {
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int get_bss_status(int *status)
{
	apcmdbuf_sys_configure *cmd_buf = NULL;
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int send_bss_ctl_ioctl(int mode)
{
	struct ifreq ifr;
	t_s32 sockfd;
 ...
 *  $return         N/A
 */
static void print_sys_reset_usage(void)
{
	printf("\nUsage : sys_reset\n");
	return;
}
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_sys_reset(int argc, char *argv[])
{
	int opt;
	int ret = UAP_SUCCESS;
	ps_mgmt pm;
 ...
 *  $return         N/A
 */
static void print_bss_start_usage(void)
{
	printf("\nUsage : bss_start\n");
	return;
}
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_bss_start(int argc, char *argv[])
{
	int opt;
	t_u8 *buf = NULL;
	t_u16 buf_len = 0;
 ...
 *  $return         N/A
 */
static void print_bss_stop_usage(void)
{
	printf("\nUsage : bss_stop\n");
	return;
}
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_bss_stop(int argc, char *argv[])
{
	int opt;
	int status = 0;
	int ret = UAP_SUCCESS;
...
/**
 *  @brief Show usage information for the skip_cac command
 *
 *  $return         N/A
 */
static void print_skip_cac_usage(void)
{
	printf("\nUsage : skip_cac <channel> <mode>\n");
	printf("\nOptions: channel - 5G dfs channel\n");
	printf("\n         mode - 1 enable skip cac\n");
	printf("\n              - 0 disable skip cac\n");
	return;
}
/**
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_skip_cac(int argc, char *argv[])
{
	int opt;
	skip_cac_para param;
	struct ifreq ifr;
	...
	if ((IS_HEX_OR_DIGIT(argv[0]) == UAP_FAILURE) ||
	    ((atoi(argv[0]) < 52) || (atoi(argv[0]) > 144))) {
		printf("ERR: Only channels 52 to 144 are allowed\n");
		print_skip_cac_usage();
		return UAP_FAILURE;
	}
	if ((IS_HEX_OR_DIGIT(argv[1]) == UAP_FAILURE) ||
	...
	param.action = 1;
	param.channel = (t_u16)A2HEXDECIMAL(argv[0]);
	param.skip_cac = (t_u16)A2HEXDECIMAL(argv[1]);
 ...
 *  $return         N/A
 */
static void print_sta_list_usage(void)
{
	printf("\nUsage : sta_list\n");
	return;
}
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_sta_list(int argc, char *argv[])
{
	struct ifreq ifr;
	t_s32 sockfd;
	sta_list *list = NULL;
	...
	list = malloc(sizeof(sta_list));
	if (!list) {
		printf("ERR: Fail to allocate sta_list buf\n");
		return UAP_FAILURE;
	}
	...
	ie_buf = (t_u8 *)&list->info[0];
	ie_buf += list->sta_count * sizeof(sta_info);
	for (i = 0; i < list->sta_count; i++) {
		...
		printf("\nPower mgmt status: %s\n",
		       (list->info[i].power_mgmt_status == 0) ? "active" :
								"power save");
		printf("Mode: %s\n",
		       (list->info[i].bandmode == BAND_B) ?
			       "11b," :
			       (list->info[i].bandmode == BAND_G) ?
			       "11g," :
			       (list->info[i].bandmode == BAND_A) ?
			       "11a," :
			       (list->info[i].bandmode == BAND_GN) ?
			       "2.4G_11n," :
			       (list->info[i].bandmode == BAND_AN) ?
			       "5G_11n," :
			       (list->info[i].bandmode == BAND_GAC) ?
			       "2.4G_11ac," :
			       (list->info[i].bandmode == BAND_AAC) ?
			       "5G_11ac," :
			       (list->info[i].bandmode == BAND_GAX) ?
			       "2.4G_11ax," :
			       (list->info[i].bandmode == BAND_AAX) ?
			       "5G_11ax," :
			       "unknown");
		...
		if (list->info[i].ie_len) {
			hexdump_data("IE", (void *)ie_buf, list->info[i].ie_len,
				     ' ');
			ie_buf += list->info[i].ie_len;
		}
	}
	/* Close socket */
	close(sockfd);
 ...
 *  $return         N/A
 */
static void print_sta_deauth_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_sta_deauth(int argc, char *argv[])
{
	...
	if ((ret = mac2raw(argv[0], cmd_buf->sta_mac_address)) != UAP_SUCCESS) {
		printf("ERR: %s Address\n",
		       ret == UAP_FAILURE ?
			       "Invalid MAC" :
			       ret == UAP_RET_MAC_BROADCAST ? "Broadcast" :
							      "Multicast");
		free(buffer);
		return UAP_FAILURE;
	}
	...
	if (ret == UAP_SUCCESS) {
		/* Verify response */
		if (cmd_buf->cmd_code !=
		    (APCMD_STA_DEAUTH | APCMD_RESP_CHECK)) {
			printf("ERR:Corrupted response!\n");
			free(buffer);
			return UAP_FAILURE;
		}
 ...
 *  $return         N/A
 */
static void print_sta_deauth_ext_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_sta_deauth_ext(int argc, char *argv[])
{
	int ret = UAP_SUCCESS;
	int opt;
	deauth_param param;
 ...
 *  $return         N/A
 */
static void print_radio_ctl_usage(void)
{
	printf("\nUsage : radioctrl [ 0 | 1 ]\n");
	return;
}
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_radio_ctl(int argc, char *argv[])
{
	int opt;
 ...
 *  $return         N/A
 */
static void print_txratecfg_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int get_tx_rate_cfg(tx_rate_cfg_t *rate_config)
{
	struct ifreq ifr;
	t_s32 sockfd;
	int ret = UAP_SUCCESS;
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_tx_rate_cfg(int argc, char *argv[])
{
	int opt;
	int status = 0;
	...
	/* Check arguments */
	if (argc > 4) {
		printf("ERR:wrong arguments! Only 0~4 arguments are supported.\n");
		print_txratecfg_usage();
		return UAP_FAILURE;
	}
	...
	if (argc) {
		...
		if (tx_rate_config.rate_format == 0xFF) {
			...
			if (tx_rate_config.rate_setting == 0xffff)
				...
				printf("BW: %x\n",
				       (tx_rate_config.rate_setting & 0x001C) >>
					       2);
				printf("LTF + GI size %x\n",
				       (tx_rate_config.rate_setting & 0x0060) >>
					       5);
				printf("STBC %x\n",
				       (tx_rate_config.rate_setting & 0x0080) >>
					       7);
				printf("DCM %x\n",
				       (tx_rate_config.rate_setting & 0x0100) >>
					       8);
				printf("Coding %x\n",
				       (tx_rate_config.rate_setting & 0x0200) >>
					       9);
				printf("maxPE %x\n",
				       (tx_rate_config.rate_setting & 0x3000) >>
					       12);
			}
		} else {
...
static int get_802_11ac_cfg(struct eth_priv_vhtcfg *vhtcfg)
{
	t_u8 *buf = NULL;
	t_u8 *pos = NULL;
	...
	pos += strlen(CMD_NXP);
	memcpy((char *)pos, "vhtcfg2 3", strlen("vhtcfg2 3"));
 ...
 *  $return         N/A
 */
static void print_sys_config_usage(void)
{
 ...
 *  $return         N/A
 */
static void print_apcmd_read_eeprom_usage(void)
{
 ...
 *  $return         N/A
 */
static void print_protocol(t_u16 protocol)
{
	switch (protocol) {
	case 0:
	case PROTOCOL_NO_SECURITY:
 ...
 *  $return         N/A
 */
static void print_wep_key(tlvbuf_wep_key *tlv)
{
	int i;
	t_u16 tlv_len;
 ...
 *  @return         UAP_SUCCESS or UAP_FAILURE
 */
static int apcmd_sys_config_profile(int argc, char *argv[])
{
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (is_ap_config == 1) {
			if (strcmp(args[0], "}") == 0) {
				...
				if (tx_beacon_rate != -1) {
					...
					cmd_buf = (apcmdbuf_sys_configure *)
						buffer;
					...
					endian_convert_tlv_header_out(tlv);
					tlv->tx_data_rate = uap_cpu_to_le16(
						tlv->tx_data_rate);
				}
				if (mcbc_data_rate != -1) {
					...
					endian_convert_tlv_header_out(tlv);
					tlv->mcbc_datarate = uap_cpu_to_le16(
						tlv->mcbc_datarate);
				}
				...
				if (((pwkcipher_wpa >= 0) ||
					...
					if ((protocol == PROTOCOL_WPA) ||
						...
						} else if (is_cipher_valid_with_proto(
								   pwkcipher_wpa,
								   gwkcipher,
								   protocol) !=
							   UAP_SUCCESS) {
				...
				if (0 == get_fw_info(&fw)) {
					/*check whether support 802.11AC through
					 * BAND_AAC bit*/
					if (fw.fw_bands & BAND_AAC) {
				...
				if (protocol != -1) {
					...
					if (protocol &
						...
						tlv_akmp->tag =
							MRVL_AKMP_TLV_ID;
						tlv_akmp->length = 4; /* sizeof(tlvbuf_akmp)
									 -
									 TLVHEADER
								       */
						if (protocol &
						...
						endian_convert_tlv_header_out(
							tlv_akmp);
						tlv_akmp->key_mgmt = uap_cpu_to_le16(
							tlv_akmp->key_mgmt);
				...
				if (ap_chan != -1) {
					...
					tlv->length =
						sizeof(tlvbuf_channel_config) -
						TLVHEADER_LEN;
					tlv->chan_number = (t_u8)ap_chan;
					if (chan_mode & BITMAP_ACS_MODE) {
						int mode;
						if (uap_ioctl_dfs_repeater_mode(
							    &mode) ==
						    UAP_SUCCESS) {
							if (mode) {
								printf("ERR: ACS in DFS Repeater mode"
								       " is not allowed\n");
								ret = UAP_FAILURE;
								goto done;
							}
					...
					if (ap_band != -1)
						tlv->bandcfg.chanBand = ap_band;
					endian_convert_tlv_header_out(tlv);
				}

				cmd_buf->size = cmd_len;
				/* Send collective command */
				if (uap_ioctl((t_u8 *)cmd_buf, &cmd_len,
					      cmd_len) == UAP_SUCCESS) {
					if (cmd_buf->result != CMD_SUCCESS) {
			...
			if (strcmp(args[0], "}") == 0) {
				...
				if (filter_tlv->count) {
					filter_tlv->length =
						...
						2;
					cmd_len -= (MAX_MAC_ONESHOT_FILTER -
						    filter_mac_count) *
						   ETH_ALEN;
				} else {
					filter_tlv->length =
		...
		if (strcmp(args[0], "11d_enable") == 0) {
			...
			if (sg_snmp_mib(ACTION_SET, OID_80211D_ENABLE,
					sizeof(state_80211d),
					&state_80211d) == UAP_FAILURE) {
				ret = UAP_FAILURE;
				goto done;
			}
		}
		...
		if (strcmp(args[0], "country") == 0) {
			apcmdbuf_cfg_80211d *cmd_80211d_buf = NULL;
			...
			no_of_sub_band = parse_domain_file(country_80211d, band,
							   sub_bands, NULL);
			...
			memset(buf, 0, buf_len);
			cmd_80211d_buf = (apcmdbuf_cfg_80211d *)buf;
			cmdlen = buf_len;
			cmd_80211d_buf->size = cmdlen - BUF_HEADER_SIZE;
			cmd_80211d_buf->result = 0;
			cmd_80211d_buf->seq_num = 0;
			cmd_80211d_buf->action = uap_cpu_to_le16(ACTION_SET);
			cmd_80211d_buf->cmd_code =
				HostCmd_CMD_802_11D_DOMAIN_INFO;
			cmd_80211d_buf->domain.tag =
				uap_cpu_to_le16(TLV_TYPE_DOMAIN);
			cmd_80211d_buf->domain.length = uap_cpu_to_le16(
				sizeof(domain_param_t) - BUF_HEADER_SIZE +
				(no_of_sub_band *
				 sizeof(ieeetypes_subband_set_t)));

			memset(cmd_80211d_buf->domain.country_code, ' ',
			       sizeof(cmd_80211d_buf->domain.country_code));
			memcpy(cmd_80211d_buf->domain.country_code,
			       country_80211d, strlen(country_80211d));
			memcpy(cmd_80211d_buf->domain.subband, sub_bands,
			       no_of_sub_band *
				       sizeof(ieeetypes_subband_set_t));

			/* Send the command */
			if (uap_ioctl((t_u8 *)cmd_80211d_buf, &cmdlen,
				      cmdlen) == UAP_SUCCESS) {
				if (cmd_80211d_buf->result != CMD_SUCCESS) {
					printf("ERR: Failed to set the configuration!\n");
					ret = UAP_FAILURE;
					goto done;
				}
		...
		if (strcmp(args[0], "BeaconPeriod") == 0) {
			if (is_input_valid(BEACONPERIOD, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
		...
		if (strcmp(args[0], "ChanList") == 0) {
			if (is_input_valid(SCANCHANNELS, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
			...
			/* Append a new TLV */
			tlv_len = sizeof(tlvbuf_channel_list) +
		...
		if (strcmp(args[0], "Channel") == 0) {
			...
			ap_chan = (t_u8)atoi(args[1]);
			if (ap_chan > MAX_CHANNELS_BG)
				band = BAND_A;
			else
				band = BAND_B | BAND_G;
			if ((arg_num - 1) == 2)
		...
		if (strcmp(args[0], "AP_MAC") == 0) {
			...
			tlv->length = ETH_ALEN;
			if ((ret = mac2raw(args[1], tlv->ap_mac_addr)) !=
			    UAP_SUCCESS) {
				printf("ERR: %s Address \n",
				       ret == UAP_FAILURE ?
					       "Invalid MAC" :
		...
		if (strcmp(args[0], "BroadcastSSID") == 0) {
			if (is_input_valid(BROADCASTSSID, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
		...
		if (strcmp(args[0], "RSNReplayProtection") == 0) {
			if (is_input_valid(RSNREPLAYPROT, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
		...
		if (strcmp(args[0], "TxBeaconRate") == 0) {
			if (is_input_valid(TXBEACONRATE, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
		...
		if (strcmp(args[0], "MCBCdataRate") == 0) {
			if (is_input_valid(MCBCDATARATE, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
		...
		if (strcmp(args[0], "StaAgeoutTimer") == 0) {
			if (is_input_valid(STAAGEOUTTIMER, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
		...
		if (strcmp(args[0], "PSStaAgeoutTimer") == 0) {
			if (is_input_valid(PSSTAAGEOUTTIMER, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
		...
		if (strcmp(args[0], "PSK") == 0) {
			if (arg_num == 1) {
				...
				tlv_len = sizeof(tlvbuf_wpa_passphrase) +
					  strlen(args[1]);
				if (strlen(args[1]) >
		...
		if (strcmp(args[0], "GroupRekeyTime") == 0) {
			if (is_input_valid(GROUPREKEYTIMER, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
 ...
 *  $return         N/A
 */
static void print_apcmd_cfg_80211d_usage(void)
{
 ...
 *  $return         N/A
 */
static void print_apcmd_uap_stats(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_uap_stats(int argc, char *argv[])
{
	...
	if (ret == UAP_SUCCESS) {
		if (cmd_buf->result == CMD_SUCCESS) {
			...
			size = cmd_buf->size -
			       (sizeof(apcmdbuf_snmp_mib) - BUF_HEADER_SIZE);
 ...
 *
 */
static void parse_input_80211d(int argc, char **argv, int output[2][2])
{
	int i, j, k = 0;
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_cfg_80211d(int argc, char *argv[])
{
	...
	if (argc) {
		...
		if ((output[1][0] != -1) && (output[1][1] > output[1][0])) {
			...
			no_of_sub_band = parse_domain_file(country, band,
							   sub_bands, NULL);
			if (no_of_sub_band == UAP_FAILURE) {
				printf("Parsing Failed\n");
				return UAP_FAILURE;
	...
	if (sflag && !cflag) {
		 ...
		 */
		if (sg_snmp_mib(ACTION_SET, OID_80211D_ENABLE, sizeof(state),
				&state) == UAP_SUCCESS) {
	...
	if (cflag) {
		...
		cmd_buf->domain.length = uap_cpu_to_le16(
			sizeof(domain_param_t) - BUF_HEADER_SIZE +
	...
	if (ret == UAP_SUCCESS) {
		if (cmd_buf->result == CMD_SUCCESS) {
			if (argc) {
				...
				if (sg_snmp_mib(ACTION_GET, OID_80211D_ENABLE,
						sizeof(state),
						&state) == UAP_SUCCESS) {
					printf("State = %sd\n",
					       state ? "enable" : "disable");
				...
				if (state) {
					t_u8 *end_buf =
						(t_u8 *)cmd_buf +
						cmd_buf->buf_size +
						sizeof(cmd_buf->buf_size);
					t_u8 *tlv_buf =
						(t_u8 *)&cmd_buf->domain +
						cmd_buf->domain.length +
						TLVHEADER_LEN;
					reg_domain_param_t *pTlv =
						(reg_domain_param_t *)tlv_buf;

					if (((end_buf - tlv_buf) >
					     TLVHEADER_LEN) &&
					    (pTlv->tag ==
					     MRVL_REGION_DOMAIN_CODE_TLV_ID)) {
						char *regDomainStr[NXP_DFS_MAX] =
							{"DFS_UNSET", "DFS_FCC",
							 "DFS_ETSI", "DFS_JP"};

						printf("DFS Regulatory Domain = %s\n",
						       (pTlv->regDomain <
							NXP_DFS_MAX) ?
							       regDomainStr
								       [pTlv->regDomain] :
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_sys_config(int argc, char *argv[])
{
	apcmdbuf_sys_configure *cmd_buf = NULL;
	t_u8 *buf = NULL;
	t_u16 cmd_len;
 ...
 * @return          UAP_SUCCESS/UAP_FAILURE
 */
static int update_domain_info(char *country_80211d, int band)
{
	apcmdbuf_cfg_80211d *cmd_buf = NULL;
	t_u8 *buf = NULL;
	t_u16 cmd_len = 0;
 ...
 *  @return             Rate index
 */
static int get_rate_index_from_bitmap(t_u16 *rate_bitmap, int size)
{
	int i;
		...
		case MRVL_CHANNELCONFIG_TLV_ID:
			...
			if (channel_tlv_band != BAND_A) {
				...
				if (secondary_ch_set == SEC_CHAN_BELOW) {
					...
					} else if (strncmp(country_80211d, "JP",
							   COUNTRY_CODE_LEN -
								   1)) {
						if (channel_tlv->chan_number >
						    DEFAULT_MAX_CHANNEL_BELOW_NON_US) {
							printf("ERR: Only channels 5-13 are allowed with secondary channel below for"
							       "non-Japan countries!\n");
							return UAP_FAILURE;
						}
					}
				...
				if (secondary_ch_set == SEC_CHAN_ABOVE) {
					if (!strncmp(country_80211d, "US",
						...
						if (channel_tlv->chan_number >
							...
							printf("ERR: Only channels 1-9 are allowed with secondary channel above for"
							       "non-Japan countries!\n");
							return UAP_FAILURE;
						}
					}
			...
			if (!(channel_tlv->bandcfg.scanMode == SCAN_MODE_ACS)) {
				if (check_channel_validity_11d(
					    ...
					    channel_tlv_band, 1) == UAP_FAILURE)
					return UAP_FAILURE;
				if (state_80211d) {
...
#define JP_REGION_0xFE 0xFE
		...
		case HT_CAPABILITY_TLV_ID:
			...
			if (ht_cap_tlv->ht_cap.supported_mcs_set[0]) {
				enable_11n = 1;
				enable_40Mhz = IS_11N_40MHZ_ENABLED(
					ht_cap_tlv->ht_cap.ht_cap_info);
				enable_20Mhz_sgi = IS_11N_20MHZ_SHORTGI_ENABLED(
					ht_cap_tlv->ht_cap.ht_cap_info);
				enable_gf = IS_11N_GF_ENABLED(
					ht_cap_tlv->ht_cap.ht_cap_info);
			}
			break;
		...
		case MRVL_MCBC_DATA_RATE_TLV_ID:
			...
			mcbc_data_rate = uap_le16_to_cpu(
				mcbc_data_rate_tlv->mcbc_datarate);
			if (mcbc_data_rate &&
	...
	if ((protocol == PROTOCOL_WPA2) || (protocol == PROTOCOL_WPA2_MIXED) ||
	    (protocol == PROTOCOL_WPA3_SAE) || (protocol == PROTOCOL_OWE)) {
	...
	if (acs_mode_enabled) {
		...
		if (!state_80211d) {
			...
			pchan_list = (channel_list *)&(chnlist_tlv->chan_list);
			for (i = 0; i < chan_list_len; i++) {
				if ((pchan_list->bandcfg.chanBand ==
				     BAND_5GHZ) &&
		...
		if (state_80211d) {
			...
			memcpy(country_80211d, "US ",
			       sizeof(country_80211d) - 1);
			pchan_list = (channel_list *)&(chnlist_tlv->chan_list);
			for (i = 0; i < chan_list_len; i++) {
				/* Set country code to JP if channel is 8 or 12
				 * and band is 5GHZ */
				if ((pchan_list->chan_number <
				     MAX_CHANNELS_BG) &&
				    (scan_channels_band == BAND_A)) {
					memcpy(country_80211d, "JP ",
					       sizeof(country_80211d) - 1);
			...
			if (update_domain_info(country_80211d,
					       scan_channels_band) ==
			    UAP_FAILURE) {
				return UAP_FAILURE;
			}
	...
	if (strncmp(dev_name, "wfd", 3))
		...
		if ((!acs_mode_enabled &&
			...
			for (i = 0; i < op_rates_len; i++) {
				rate = rates_tlv->operational_rates[i] &
				       ~BASIC_RATE_SET_BIT;
				switch (rate) {
				case 2:
					rate_bitmap |= BITMAP_RATE_1M;
	...
	if ((!acs_mode_enabled && (channel_tlv_band == BAND_A)) ||
		...
		state_80211h = 1;
		if (sg_snmp_mib(ACTION_SET, OID_80211H_ENABLE,
				sizeof(state_80211h),
				&state_80211h) == UAP_FAILURE) {
			return UAP_FAILURE;
		}
...
#define ISSUPP_CHANWIDTH40(Dot11nDevCap) (Dot11nDevCap & MBIT(17))
	...
	if (enable_11n) {
		/*For protocol = Mixed, 11n enabled, only allow TKIP cipher for
		 * WPA protocol, not for WPA2 */
		if ((protocol == PROTOCOL_WPA2_MIXED) &&
	...
	if (!acs_mode_enabled && enable_40Mhz) {
		if ((channel == 14) ||
		    (channel == 165 &&
		     strncmp(country_80211d, "US", COUNTRY_CODE_LEN - 1))) {
...
#define B_RATE_MANDATORY 0x0f
	...
	if (channel_tlv_band != BAND_A) {
		...
		/* When country code is not Japan, allow channels 1-7 for US and
		 * 1-9 for non-US only for secondary channel above */
		if (secondary_ch_set == SEC_CHAN_ABOVE) {
			if (!strncmp(country_80211d, "US",
				     COUNTRY_CODE_LEN - 1)) {
				if (bss_config->channel >
	...
	if (!(bss_config->bandcfg.scanMode == SCAN_MODE_ACS)) {
		if (check_channel_validity_11d(bss_config->channel,
					       channel_tlv_band,
					       1) == UAP_FAILURE)
			return UAP_FAILURE;
		if (state_80211d) {
			/* Set country code to JP if channel is 8 or 12 and band
			 * is 5GHZ */
			if ((bss_config->channel < MAX_CHANNELS_BG) &&
			    (channel_tlv_band == BAND_A))
				memcpy(country_80211d, "JP ",
...
#define JP_REGION_0xFE 0xFE
	...
	for (i = 0; i < (int)bss_config->num_of_chan; i++) {
		scan_channels_band = BAND_B | BAND_G;
		if ((scan_channels_band != BAND_A) &&
	...
	if ((bss_config->protocol == PROTOCOL_WPA) ||
	    (bss_config->protocol == PROTOCOL_WPA2_MIXED)) {
		if (is_cipher_valid(bss_config->wpa_cfg.pairwise_cipher_wpa,
				    bss_config->wpa_cfg.group_cipher) !=
		    UAP_SUCCESS) {
			printf("ERR:Wrong group cipher and WPA pairwise cipher combination!\n");
			return UAP_FAILURE;
		}
	...
	if ((bss_config->protocol == PROTOCOL_WPA2) ||
	    (bss_config->protocol == PROTOCOL_WPA2_MIXED) ||
	    (bss_config->protocol == PROTOCOL_WPA3_SAE) ||
	    (bss_config->protocol == PROTOCOL_OWE)) {
		if (is_cipher_valid(bss_config->wpa_cfg.pairwise_cipher_wpa2,
				    bss_config->wpa_cfg.group_cipher) !=
		    UAP_SUCCESS) {
			printf("ERR:Wrong group cipher and WPA2 pairwise cipher combination!\n");
			return UAP_FAILURE;
	...
	if (acs_mode_enabled) {
		...
		if (!state_80211d) {
			 ...
			 * not enabled and country code not set to JP */
			for (i = 0; i < (int)bss_config->num_of_chan; i++) {
				if ((bss_config->chan_list[i].bandcfg.chanBand ==
				     BAND_5GHZ) &&
				    ((bss_config->chan_list[i].chan_number ==
				      8) ||
				     (bss_config->chan_list[i].chan_number ==
 ...
 *  @return         UAP_SUCCESS or UAP_FAILURE
 */
static int apcmd_regrdwr_process(int reg, char *offset, char *strvalue)
{
	...
	if (ret == UAP_SUCCESS) {
		if (cmd_buf->result == CMD_SUCCESS) {
			...
			printf("%s[0x%04hx] = 0x%08x\n", whichreg,
			       uap_le16_to_cpu(cmd_buf->offset),
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_read_eeprom(int argc, char *argv[])
{
 ...
 *  $return         N/A
 */
static void print_regrdwr_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_regrdwr(int argc, char *argv[])
{
	int opt;
	t_s32 reg;
	int ret = UAP_SUCCESS;
 ...
 *    $return         N/A
 */
static void print_memaccess_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_memaccess(int argc, char *argv[])
{
	int opt;
 ...
 *  $return         N/A
 */
static void print_bss_config_usage(void)
{
 ...
 *  @return         UAP_SUCCESS or UAP_FAILURE
 */
static int parse_bss_config(int argc, char *argv[], bss_config_t *bss)
{
	FILE *config_file = NULL;
	char *line = NULL;
	int li = 0;
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "AP_MAC") == 0) {
			int ret;
			if ((ret = mac2raw(args[1], bss->mac_addr)) !=
			    UAP_SUCCESS) {
				printf("ERR: %s Address \n",
				       ret == UAP_FAILURE ?
					       "Invalid MAC" :
		...
		if (strcmp(args[0], "PreambleType") == 0) {
			if (is_input_valid(PREAMBLETYPE, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				retval = UAP_FAILURE;
				goto done;
			}
		...
		if (strcmp(args[0], "HTCapInfo") == 0) {
			...
			bss->ht_cap_info =
				DEFAULT_HT_CAP_VALUE & ~HT_CAP_CONFIG_MASK;
			bss->ht_cap_info |= (t_u16)A2HEXDECIMAL(args[1]) &
					    HT_CAP_CONFIG_MASK;
 ...
 *  $return         N/A
 */
static void print_bss_config(bss_config_t *buf)
{
	int i = 0;
	int flag = 0;
	...
	printf("%s handles packet forwarding -\n",
	       ((buf->pkt_forward_ctl & PKT_FWD_FW_BIT) == 0) ? "Host" :
								"Firmware");
	printf("\tIntra-BSS broadcast packets are %s\n",
	       ((buf->pkt_forward_ctl & PKT_FWD_INTRA_BCAST) == 0) ? "allowed" :
								     "denied");
	printf("\tIntra-BSS unicast packets are %s\n",
	       ((buf->pkt_forward_ctl & PKT_FWD_INTRA_UCAST) == 0) ? "allowed" :
								     "denied");
	printf("\tInter-BSS unicast packets are %s\n",
	       ((buf->pkt_forward_ctl & PKT_FWD_INTER_UCAST) == 0) ? "allowed" :
								     "denied");
	...
	printf("Preamble type = %s\n",
	       (buf->preamble_type == 0) ?
		       "auto" :
		       ((buf->preamble_type == 1) ? "short" : "long"));
	...
	for (i = 0; (unsigned int)i < buf->num_of_chan; i++) {
		printf("\n%d\t%sGHz", buf->chan_list[i].chan_number,
		       (buf->chan_list[i].bandcfg.chanBand == BAND_5GHZ) ?
			       "5" :
			       "2.4");
	}
	printf("\n");
	...
	case 3:
		...
		/* used for embedded supplicant */
		printf("PWE derivation = %d\n", buf->pwe_derivation);
		printf("Transition Disable = %d\n", buf->transition_disable);
		break;
	case 4:
		printf("AUTHMODE = OWE\n");
		break;
	...
	if (buf->wep_cfg.key0.length) {
		...
		(buf->wep_cfg.key0.is_default) ? (printf("<Default>\n")) :
						 (printf("\n"));
	} else {
	...
	if (buf->wep_cfg.key1.length) {
		...
		(buf->wep_cfg.key1.is_default) ? (printf("<Default>\n")) :
						 (printf("\n"));
	} else {
	...
	if (buf->wep_cfg.key2.length) {
		...
		(buf->wep_cfg.key2.is_default) ? (printf("<Default>\n")) :
						 (printf("\n"));
	} else {
	...
	if (buf->wep_cfg.key3.length) {
		...
		(buf->wep_cfg.key3.is_default) ? (printf("<Default>\n")) :
						 (printf("\n"));
	} else {
	...
	if (buf->protocol &
	    (PROTOCOL_WPA2 | PROTOCOL_WPA3_SAE | PROTOCOL_OWE)) {
...
static int get_bss_config(t_u8 *buf)
{
	...
	hexdump("Get Request buffer", (void *)buf,
		sizeof(apcmdbuf_bss_configure) + sizeof(bss_config_t), ' ');
#endif
	/* Initialize the ifr structure */
	memset(&ifr, 0, sizeof(ifr));
	strncpy(ifr.ifr_ifrn.ifrn_name, dev_name, IFNAMSIZ - 1);
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_bss_config(int argc, char *argv[])
{
	apcmdbuf_bss_configure *cmd_buf = NULL;
 ...
 *  @return         UAP_SUCCESS or UAP_FAILURE
 */
static int apcmd_coex_config_profile(int argc, char *argv[])
{
	FILE *config_file = NULL;
	char *line = NULL;
	int i, index, li = 0;
	...
	t_u16 sco_prot_qtime[4] = {0, 0, 0, 0}, sco_prot_rate = 0,
	      sco_acl_freq = 0;
	...
	cmd_len = sizeof(apcmdbuf_coex_config) +
		  sizeof(tlvbuf_coex_common_cfg) + sizeof(tlvbuf_coex_sco_cfg) +
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (is_coex_acl_config == 1) {
			if (strcmp(args[0], "}") == 0) {
				coex_acl_tlv = (tlvbuf_coex_acl_cfg
							*)(cmd_buf->tlv_buffer +
							   tlv_len);
			...
			if (strcmp(args[0], "}") == 0) {
				coex_sco_tlv = (tlvbuf_coex_sco_cfg
							*)(cmd_buf->tlv_buffer +
							   tlv_len);
		...
		if ((strcmp(args[0], "bitmap") == 0) && is_coex_common_config) {
			if (is_input_valid(COEX_COMM_BITMAP, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
			...
			if (is_input_valid(COEX_COMM_AP_COEX, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
			...
			sco_prot_rate = (t_u16)atoi(args[1]);
			   ...
			   is_coex_sco_config) {
			if (is_input_valid(COEX_SCO_ACL_FREQ, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
			sco_acl_freq = (t_u16)atoi(args[1]);
			   ...
			   is_coex_acl_config) {
			if (is_input_valid(COEX_ACL_ENABLED, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
			acl_enabled = (t_u16)atoi(args[1]);
			   ...
			   is_coex_acl_config) {
			if (is_input_valid(COEX_ACL_BT_TIME, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
			acl_bt_time = (t_u16)atoi(args[1]);
			   ...
			   is_coex_acl_config) {
			if (is_input_valid(COEX_ACL_WLAN_TIME, arg_num - 1,
					   args + 1) != UAP_SUCCESS) {
				ret = UAP_FAILURE;
				goto done;
			}
 ...
 *  $return         N/A
 */
static void print_coex_config_usage(void)
{
 ...
 *  @return         UAP_SUCCESS or UAP_FAILURE
 */
static int apcmd_coex_config(int argc, char *argv[])
{
	apcmdbuf_coex_config *cmd_buf = NULL;
	tlvbuf_coex_common_cfg *coex_common_tlv;
	tlvbuf_coex_sco_cfg *coex_sco_tlv;
	...
	cmd_len = sizeof(apcmdbuf_coex_config) +
		  sizeof(tlvbuf_coex_common_cfg) + sizeof(tlvbuf_coex_sco_cfg) +
		  sizeof(tlvbuf_coex_acl_cfg) + sizeof(tlvbuf_coex_stats);
	/* alloc buf for command */
	buf = (t_u8 *)malloc(buf_len);
	if (!buf) {
		printf("ERR:Cannot allocate buffer from command!\n");
 ...
 *  $return         N/A
 */
static void print_mic_err_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_mic_err(int argc, char *argv[])
{
	int ret = UAP_SUCCESS;
	int opt;
	struct ifreq ifr;
 ...
 *  $return         N/A
 */
static void print_set_key_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_set_key(int argc, char *argv[])
{
	int ret = UAP_SUCCESS;
	int opt;
	struct ifreq ifr;
 ...
 *  $return         N/A
 */
static void print_sys_cfg_custom_ie_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int get_max_mgmt_ie(int *max_mgmt_ie, int flag)
{
	apcmdbuf_sys_configure *cmd_buf = NULL;
...
int apcmd_sys_cfg_custom_ie(int argc, char *argv[])
{
	...
	if (argc == 1 && max_mgmt_ie_print == 0) {
		 ...
		 * one by one
		 * && (max_mgmt_ie_print == 0) i.e. max mgmt IE is not printed
		 * via sys_config cmd */
		print_flag = 1;
	}
			...
			if (tlv->tag == MRVL_MGMT_IE_LIST_TLV_ID) {
				while ((unsigned int)ie_len >=
					...
					hexdump_data("IE Buffer",
						     ...
						     (ie_ptr->ie_length), ' ');
					ie_len -= sizeof(custom_ie) +
						  ie_ptr->ie_length;
					ie_ptr =
						(custom_ie *)((t_u8 *)ie_ptr +
							      sizeof(custom_ie) +
							      ie_ptr->ie_length);
				}
			}
 ...
 *  $return         N/A
 */
static void print_dfstesting_usage(void)
{
	...
	printf("\n                 0       - disable, use default (60)");
	printf("\n                 1-1800 - CAC period in sec");
	...
	printf("\n         CAC_RESTART:  user configure auto restart cac after radar detected");
	printf("\n                 0     - disable cac restart [default]");
	printf("\n                 1     - enable restart cac after radar detected\n");
 ...
 *
 *   Usage: "dfstesting [USER_CAC_PD  USER_NOP_PD  NO_CHAN_CHANGE
 * FIXED_CHAN_NUM]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_dfstesting(int argc, char *argv[])
{
	int opt;
	struct ifreq ifr;
	t_s32 sockfd;
	...
	if (argc == 0) {
		...
		val = A2HEXDECIMAL(argv[4]);
		dfs_test.cac_restart = (t_u8)val;
 ...
 *  $return         N/A
 */
static void print_cscount_cfg_usage(void)
{
	printf("\nUsage : cscount [<n>]");
	printf("\n        Where  <n>  ");
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_cscount_cfg(int argc, char *argv[])
{
	int opt;
 ...
 *  $return         N/A
 */
static void print_mgmtframectrl_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_mgmt_frame_control(int argc, char *argv[])
{
	int opt;
 ...
 *  $return         N/A
 */
static void print_sys_cfg_tdls_ext_cap_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int get_tdls_ext_cap(tlvbuf_tdls_ext_cap *ext_cap_tlv)
{
	apcmdbuf_sys_configure *cmd_buf = NULL;
	...
	buf_len = sizeof(apcmdbuf_sys_configure) + sizeof(tlvbuf_tdls_ext_cap) +
	...
	tlv->length = 0;
	cmd_len = sizeof(apcmdbuf_sys_configure) + sizeof(tlvbuf_tdls_ext_cap) +
		  tlv->length;
	/* Fill the command buffer */
	cmd_buf->cmd_code = APCMD_SYS_CONFIGURE;
	cmd_buf->size = cmd_len;
 ...
 *  @return         UAP_SUCCESS or UAP_FAILURE
 */
static int parse_tdls_config(int argc, char *argv[],
			     tlvbuf_tdls_ext_cap *ext_cap_tlv)
{
	FILE *config_file = NULL;
	char *line = NULL;
	char *pos = NULL;
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_sys_cfg_tdls_ext_cap(int argc, char *argv[])
{
	apcmdbuf_sys_configure *cmd_buf = NULL;
	tlvbuf_tdls_ext_cap *tlv = NULL;
	t_u8 *buffer = NULL;
		...
		if (cmd_buf->result == CMD_SUCCESS) {
			if (argc == 0) {
				printf("TDLS prohibited = %s\n",
				       (tlv->ext_cap[4] & TDLS_PROHIBIT_MASK) ?
					       "YES" :
					       "NO");
 ...
 *    $return         N/A
 */
static void print_sys_cfg_pmf(void)
{
...
static int apcmd_sys_cfg_pmf(int argc, char *argv[])
{
	int opt;
 ...
 *  $return         N/A
 */
static void print_uap_oper_ctrl_usage(void)
{
 ...
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int apcmd_uap_oper_ctrl(int argc, char *argv[])
{
	int opt;
	...
	if (param.action == ACTION_GET) {
		/* Process result */
		uap_oper = (struct eth_priv_uap_oper_ctrl *)(respbuf +
							     2 * sizeof(t_u32));
...
static command_table ap_command[] = {
	...
	{"beacon_stuck_detect", apcmd_beacon_stuck_detect,
	 "\tConfigure Beacon Stuck Parameters."},
 ...
 *  @return          N/A
 */
static void print_tool_usage(void)
{
	int i;
...
****************************************************************************/
/** Option parameter*/
static struct option ap_options[] = {{"help", 0, NULL, 'h'},
				     {"interface", 1, NULL, 'i'},
				     {"debug", 1, NULL, 'd'},
				     {"version", 0, NULL, 'v'},
				     {NULL, 0, NULL, '\0'}};
 ...
 * @return          UAP_SUCCESS or UAP_FAILURE
 */
static int is_valid_a_band_channel(int channel)
{
	int ret = UAP_SUCCESS;
	switch (channel) {
	case 16:
...
/**
 * @brief Checks if secondary channel can be set above given primary channel in
 * 'a' band or not.
 *
 * @param channel   Channel number
 * @return          UAP_SUCCESS or UAP_FAILURE
 */
static int is_valid_a_band_channel_above(int channel)
{
	int ret = UAP_SUCCESS;
	switch (channel) {
	case 36:
...
/**
 * @brief Checks if secondary channel can be set below given primary channel in
 * 'a' band or not.
 *
 * @param channel   Channel number
 * @return          UAP_SUCCESS or UAP_FAILURE
 */
static int is_valid_a_band_channel_below(int channel)
{
	int ret = UAP_SUCCESS;
	switch (channel) {
	case 40:
...
int is_input_valid(valid_inputs cmd, int argc, char *argv[])
{
	...
	case AKM_SUITE:
		if (argc == 2) {
			if (A2HEXDECIMAL(argv[1]) &
			    ~(KEY_MGMT_PSK | KEY_MGMT_PSK_SHA256 |
			      KEY_MGMT_EAP | KEY_MGMT_NONE | KEY_MGMT_OWE |
			      KEY_MGMT_SAE)) {
	...
	case RATE:
		if (argc > MAX_RATES) {
			...
			for (i = 0; i < argc; i++) {
				if ((IS_HEX_OR_DIGIT(argv[i]) == UAP_FAILURE) ||
				    (is_rate_valid(A2HEXDECIMAL(argv[i]) &
						   ~BASIC_RATE_SET_BIT) !=
				     UAP_SUCCESS)) {
		...
		if (argc != 1) {
			...
			if (IS_HEX_OR_DIGIT(argv[0]) == UAP_FAILURE) {
				...
				ret = UAP_FAILURE;
				   ...
				   (is_rate_valid(A2HEXDECIMAL(argv[0]) &
						  ~BASIC_RATE_SET_BIT) !=
				    UAP_SUCCESS)) {
				printf("ERR: invalid data rate\n");
				ret = UAP_FAILURE;
	...
	case STAAGEOUTTIMER:
		if (argc != 1) {
			...
			if ((ISDIGIT(argv[0]) == 0) ||
			    ((atoi(argv[0]) != 0) &&
			     ((atoi(argv[0]) < MIN_STAGE_OUT_TIME) ||
			      (atoi(argv[0]) > MAX_STAGE_OUT_TIME)))) {
	...
	case AUTHMODE:
		...
		if (argc > 3) {
			...
			if ((ISDIGIT(argv[0]) == 0) ||
			    ((atoi(argv[0]) != 0) && (atoi(argv[0]) != 1) &&
			     (atoi(argv[0]) != 3) && (atoi(argv[0]) != 4) &&
			     (atoi(argv[0]) != 255))) {
				printf("ERR:Illegal AUTHMODE parameter %s. Must be either '0','1','3','4' or 255''.\n",
				       argv[0]);
				ret = UAP_FAILURE;
			}
			/* used for embedded supplicant */
			if ((argc > 1) &&
			    ((ISDIGIT(argv[1]) == 0) || (atoi(argv[1]) < 0) ||
			     (atoi(argv[1]) > 2))) {
				printf("ERR:Illegal PWE derivation parameter. Must be either '0','1' or '2'\n");
				ret = UAP_FAILURE;
			}
			if ((argc > 2) &&
			    ((ISDIGIT(argv[2]) == 0) || (atoi(argv[2]) < 0) ||
			     (atoi(argv[2]) > 1))) {
				printf("ERR:Illegal transition_disable parameter. Must be either '0' or '1'\n");
				ret = UAP_FAILURE;
			}
		}
		break;
	...
	case EAPOL_PWK_HSK:
		if (argc != 2) {
			...
			if ((ISDIGIT(argv[0]) == 0) ||
			    (ISDIGIT(argv[1]) == 0) || (atoi(argv[0]) < 0) ||
			    (atoi(argv[1]) < 0)) {
	...
	case AP_EXT_CAP_LEN:
		if (argc != 1) {
			printf("ERR:Incorrect number of arguments for EXT_CAP_LEN\n");
			ret = UAP_FAILURE;
		} else if ((ISDIGIT(argv[0]) == 0) ||
			   ((atoi(argv[0]) != 8) && (atoi(argv[0]) != 10))) {
			printf("ERR: EXT_CAP_LEN Value must be either 8 or 10.\n");
			ret = UAP_FAILURE;
		}
		break;
	default:
...
void print_tlv(t_u8 *buf, t_u16 len)
{
	...
	while (tlv_buf_left >= (int)sizeof(tlvbuf_header)) {
		...
		case MRVL_CHANNELCONFIG_TLV_ID:
			...
			printf("Channel = %d\n", channel_tlv->chan_number);
			printf("Band = %s\n",
			       (channel_tlv->bandcfg.chanBand == BAND_5GHZ) ?
				       "5GHz" :
				       "2.4GHz");
			printf("Channel Select Mode = %s\n",
		...
		case MRVL_RSN_REPLAY_PROT_TLV_ID:
			...
			printf("RSN replay protection = %s\n",
			       replay_prot_tlv->rsn_replay_prot ? "enabled" :
								  "disabled");
			break;
		case MRVL_RATES_TLV_ID:
			rates_tlv = (tlvbuf_rates *)pcurrent_tlv;
		...
		case MRVL_BCAST_SSID_CTL_TLV_ID:
			...
			printf("SSID broadcast = %s\n",
			       (bcast_tlv->bcast_ssid_ctl == 1) ? "enabled" :
								  "disabled");
			break;
		...
		case MRVL_BSS_STATUS_TLV_ID:
			...
			printf("BSS status = %s\n",
			       (bss_status_tlv->bss_status == 0) ? "stopped" :
								   "started");
			break;
		...
		case MRVL_PKT_FWD_CTL_TLV_ID:
			...
			printf("%s handles packet forwarding -\n",
				...
				0) ?
				       ...
				       "Firmware");
			printf("\tIntra-BSS broadcast packets are %s\n",
			       ((pkt_fwd_tlv->pkt_fwd_ctl &
				 PKT_FWD_INTRA_BCAST) == 0) ?
				       "allowed" :
				       "denied");
			printf("\tIntra-BSS unicast packets are %s\n",
			       ((pkt_fwd_tlv->pkt_fwd_ctl &
				 PKT_FWD_INTRA_UCAST) == 0) ?
				       "allowed" :
				       "denied");
			printf("\tInter-BSS unicast packets are %s\n",
			       ((pkt_fwd_tlv->pkt_fwd_ctl &
				 PKT_FWD_INTER_UCAST) == 0) ?
				       "allowed" :
				       "denied");
			break;
		...
		case MRVL_PS_STA_AGEOUT_TIMER_TLV_ID:
			...
			tlv_val_32 |= (*((t_u8 *)&ps_ageout_tlv
						 ->ps_sta_ageout_timer_ms +
					 1)
				       << 8);
			tlv_val_32 |= (*((t_u8 *)&ps_ageout_tlv
						 ->ps_sta_ageout_timer_ms +
		...
		case MRVL_EAPOL_PWK_HSK_TIMEOUT_TLV_ID:
			...
			pwk_timeout_tlv
				->pairwise_update_timeout = uap_le32_to_cpu(
		...
		case MRVL_EAPOL_GWK_HSK_TIMEOUT_TLV_ID:
			...
			gwk_timeout_tlv
				->groupwise_update_timeout = uap_le32_to_cpu(
		...
		case MRVL_MGMT_IE_LIST_TLV_ID:
			...
			while (custom_ie_len >= sizeof(custom_ie)) {
				custom_ie_ptr->ie_index = uap_le16_to_cpu(
					custom_ie_ptr->ie_index);
				custom_ie_ptr->ie_length = uap_le16_to_cpu(
					custom_ie_ptr->ie_length);
				custom_ie_ptr
					->mgmt_subtype_mask = uap_le16_to_cpu(
				...
				hexdump_data("IE Buffer",
					     ...
					     (custom_ie_ptr->ie_length), ' ');
				custom_ie_len -= sizeof(custom_ie) +
						 custom_ie_ptr->ie_length;
				custom_ie_ptr =
		...
		case MRVL_MAX_MGMT_IE_TLV_ID:
			...
			for (i = 0; i < max_mgmt_ie_tlv->count; i++) {
				max_mgmt_ie_tlv->info[i].buf_size =
					uap_le16_to_cpu(max_mgmt_ie_tlv->info[i]
								.buf_size);
		...
		case HT_INFO_TLV_ID:
			...
			if (ht_info_tlv->length) {
				...
				printf("Secondary channel offset = %d\n",
				       (int)GET_SECONDARY_CHAN(
					       ht_info_tlv->ht_info.field2));
				...
				printf("RIFS  %s\n",
				       IS_RIFS_ALLOWED(
					       ...
					       "Allowed" :
					       "Prohibited");
				ht_info_tlv->ht_info.field3 = uap_le16_to_cpu(
					ht_info_tlv->ht_info.field3);
				ht_info_tlv->ht_info.field4 = uap_le16_to_cpu(
					ht_info_tlv->ht_info.field4);
				printf("HT Protection = %d\n",
				       (int)GET_HT_PROTECTION(
					       ht_info_tlv->ht_info.field3));
 ...
 *  @return              UAP_SUCCESS (handled here, further processing not
 * needed) or UAP_FAILURE (not handled here, proceed as usual)
 */
static int uap_ioctl_reroute(t_u8 *cmd, t_u16 *size, t_u16 buf_size,
			     int *reroute_ret)
{
	t_u8 reroute = 0;
	t_u16 cmd_code = 0;
	...
	case HostCmd_CMD_802_11D_DOMAIN_INFO:
		...
		if (cmd_action == ACTION_SET) {
			...
			buf_len = sizeof(domain_info_param) +
				  (*size - sizeof(apcmdbuf_cfg_80211d) +
 ...
 *  @return         UAP_SUCCESS or UAP_FAILURE
 */
int is_cipher_valid_with_11n(int pairwisecipher, int groupcipher, int protocol,
			     int enable_11n)
{
...
int main(int argc, char *argv[])
{
	...
	/* Parse arguments */
	while ((opt = getopt_long(argc, argv, "+hi:d:v", ap_options, NULL)) !=
	       -1) {
		switch (opt) {
		case 'i':
