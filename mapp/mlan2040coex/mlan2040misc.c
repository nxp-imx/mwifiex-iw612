...
static class_chan_t us_class_chan_t[] = {{32, {1, 2, 3, 4, 5, 6, 7}, 7},
...
static region_class_chan_t region_class_chan_table[] = {
	{0x10, us_class_chan_t,
	 sizeof(us_class_chan_t) / sizeof(class_chan_t)} /* US */
	,
	{0x20, us_class_chan_t,
	 sizeof(us_class_chan_t) / sizeof(class_chan_t)} /* CANADA */
	,
	{0x30, europe_class_chan_t,
	 sizeof(europe_class_chan_t) / sizeof(class_chan_t)} /* EUROPE */
	,
	{0x32, europe_class_chan_t,
	 sizeof(europe_class_chan_t) / sizeof(class_chan_t)} /* FRANCE */
	,
	{0x40, japan_class_chan_t,
	 sizeof(japan_class_chan_t) / sizeof(class_chan_t)} /* JAPAN */
	,
	{0x41, japan_class_chan_t,
	 sizeof(japan_class_chan_t) / sizeof(class_chan_t)} /* JAPAN */
	,
	{0x50, europe_class_chan_t,
	 sizeof(europe_class_chan_t) / sizeof(class_chan_t)} /* CHINA */
};
 ...
 *  @param chan_num	             total number of channel in output channel
 * list
	...
	*is_intol_ap_present = FALSE;
	...
	for (i = 0; i < num_entry; i++) {
		if (cur_class_chan_table[i].reg_class == reg_class) {
			...
			for (j = 0; j < cur_class_chan_table[i].total_chan;
			     j++) {
...
int invoke_coex_command(void)
{
	...
	for (i = 0; (unsigned int)i < (sizeof(region_class_chan_table) /
				       sizeof(region_class_chan_t));
	     i++) {
	...
	for (i = 0; i < num_entry; i++) {
		...
		get_channels_for_specified_reg_class(
			cur_class_chan_table, num_entry, chan_list,
			&num_of_chan, cur_reg_domain,
			cur_class_chan_table[i].reg_class,
			&is_intol_ap_present);

		/** If any channel found for this regulatory class, then invoke
		 * the 2040coex command */
		if (num_of_chan > 0) {
			ret = process_host_cmd(
				CMD_2040COEX, chan_list, num_of_chan,
				cur_class_chan_table[i].reg_class,
				is_intol_ap_present);
			if (ret)
				break;
