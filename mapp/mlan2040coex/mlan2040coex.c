 ...
 */
static int process_scantable(void)
{
	...
	do {
		...
		for (idx = 0; (unsigned int)idx < prsp_info->scan_number;
			...
			memcpy((t_u8 *)&bss_info_len, (t_u8 *)pcurrent,
			       sizeof(bss_info_len));
			...
			if (bss_info_len >=
			    (t_s32)(sizeof(tsf) + sizeof(beacon_interval) +
				    sizeof(cap_info))) {
				pcurrent +=
			...
			while (bss_info_len >= 2) {
				...
				case HT_CAPABILITY:
					pht_cap = (IEEEtypes_HTCap_t *)
						pelement_id;
					ht_cap_present = TRUE;
					if (IS_INTOL_BIT_SET(le16_to_cpu(
						    pht_cap->ht_cap
							    .ht_cap_info))) {
 ...
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int get_scan_cfg(int *scan_param)
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
 ...
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int process_setuserscan(void)
{
	...
	memset(scan_cfg, 0x00, sizeof(scan_cfg));
	...
	if (scan_cfg[0] == SCAN_TYPE_ACTIVE)
		scan_req.chan_list[0].scan_time = (t_u32)le16_to_cpu(
			cur_obss_scan_param.obss_scan_active_dwell);
	else
		scan_req.chan_list[0].scan_time = (t_u32)le16_to_cpu(
			cur_obss_scan_param.obss_scan_passive_total);
 ...
 *  @return         N/A
 */
static void print_event(event_header *event, t_u16 size, int *evt_conn,
			char *if_name)
{
	if (!strncmp(CUS_EVT_AP_CONNECTED, (char *)event,
	...
	if (!strncmp(CUS_EVT_OBSS_SCAN_PARAM, (char *)event,
	...
	if (!strncmp(CUS_EVT_BW_CHANGED, (char *)event,
...
static void run_app(int nl_sk)
{
	...
	while (!terminate_flag) {
		...
		drv_nlevt_handler((struct nlmsghdr *)nlh, bytes_read,
				  &evt_conn);
		if (assoc_flag && is_ht_ap) {
			/** If the event is connected with an HT AP then issue
			 * OBSS scan immediately */
			if (evt_conn) {
				/** Issue OBSS scan */
				process_setuserscan();
				/** Invoke 2040coex command, if any legacy AP
				 * found or
				 *  any AP has 40MHz intolarent bit set */
				if (coex_cmd_req_flag)
					invoke_coex_command();
			}
			tv.tv_sec = (t_u32)le16_to_cpu(
				cur_obss_scan_param
					.bss_chan_width_trigger_scan_int);
		} else {
			tv.tv_sec = DEFAULT_SCAN_INTERVAL;
...
static int get_netlink_num(int dev_index)
{
	...
	if (dev_index >= 0) {
		...
		/* buffer = CMD_NXP + <cmd_string>*/
		prepare_buffer(buffer, "getnlnum", 0, NULL);
		...
		if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
			perror("mlan2040coex");
			fprintf(stderr, "mlan2040coex: getnlnum fail\n");
			goto done;
		}
 ...
 *  @return         NA
 */
static t_void terminate_handler(int signal)
{
 ...
 *  @param is_intol_ap_present Flag:is there any 40 MHz intolerant AP is present
 * or not
 *
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
int process_host_cmd(int hostcmd_idx, t_u8 *chan_list, t_u8 chan_num,
		     t_u8 reg_class, t_u8 is_intol_ap_present)
{
	...
	switch (hostcmd_idx) {
	case CMD_2040COEX:
		prepare_coex_cmd_buff(
			buffer + strlen(CMD_NXP) + strlen("hostcmd"), chan_list,
			chan_num, reg_class, is_intol_ap_present);
		break;
	default:
		break;
	}
 ...
 *  @param intol	Flag: TRUE when the STA is 40 MHz intolerant, otherwise
 * FALSE
 *  @return      	MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
...
int main(int argc, char *argv[])
{
	...
	int dev_index = -1; /** initialise with -1 to open multiple NETLINK
			       Sockets */
