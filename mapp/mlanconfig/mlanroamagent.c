 ...
 */
static int roamGetFailureCount(int argc, char *argv[])
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamGetPreBeaconMiss(int argc, char *argv[])
{
	int ioctl_val, subioctl_val, cmd_len;
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamGetStatsThreshold(int argc, char *argv[], int type)
{
	 ...
	 */
	cmd_len = cmdresplen =
		sizeof(HostCmd_DS_GEN) + sizeof(statsthreshold->action) +
			...
			case TLV_TYPE_RSSI_HIGH:
				...
				pHighRssiThreshold =
					(MrvlIEtypes_BeaconHighRssiThreshold_t
						 *)(tlvptr);
				tlvptr += (le16_to_cpu(pHighRssiThreshold
							       ->Header.len) +
					   sizeof(MrvlIEtypesHeader_t));
				i -= (le16_to_cpu(
			...
			case TLV_TYPE_RSSI_LOW:
				...
				pLowRssiThreshold =
					(MrvlIEtypes_BeaconLowRssiThreshold_t
						 *)(tlvptr);
				tlvptr += (le16_to_cpu(pLowRssiThreshold->Header
							       .len) +
					   sizeof(MrvlIEtypesHeader_t));
				i -= (le16_to_cpu(
			...
			case TLV_TYPE_SNR_HIGH:
				...
				pHighSnrThreshold =
					(MrvlIEtypes_BeaconHighSnrThreshold_t
						 *)(tlvptr);
				tlvptr += (le16_to_cpu(pHighSnrThreshold->Header
							       .len) +
					   sizeof(MrvlIEtypesHeader_t));
				i -= (le16_to_cpu(
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamGetRssiStatsThreshold(int argc, char *argv[])
{
 ...
 *  @return         none
 */
static void ms2exactTime(t_u32 ms, ExactTime_t *t)
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int getNborAssessment(
	HostCmd_DS_CMD_ROAMAGENT_NEIGHBOR_ASSESSMENT *pNborAssessmentParam)
{
	...
	cmd_len += sizeof(pNborAssessment->action);

	cmdresplen = sizeof(HostCmd_DS_GEN);
	cmdresplen += sizeof(HostCmd_DS_CMD_ROAMAGENT_NEIGHBOR_ASSESSMENT);
 ...
 *  @return         None
 */
static void printExactTime2stdout(ExactTime_t *t)
{
	int flag = 0;
	if (t->hrs) {
		printf("%dh ", t->hrs);
...
static int printNeighborAssessmentConfig(void)
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamGetNborList(int argc, char *argv[])
{
	...
	while (cmdresplen > 2) {
		...
		case TLV_TYPE_NEIGHBOR_ENTRY:
			...
			if ((pNeighbor->QualifiedNeighborBitmap &
			     (BIT_NEIGHFLAG_RSSI | BIT_NEIGHFLAG_AGE |
			...
			printf(" %02x:%02x:%02x:%02x:%02x:%02x | %3d  | %5d | %9s | ",
			       ...
			       pNeighbor->SignalStrength, pNeighbor->Age,
			       neighflag);
...
static int roamGetNborParams(int argc, char *argv[])
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamGetMetrics(int argc, char *argv[])
{
	...
	if (le16_to_cpu(hostcmd->result) == MLAN_STATUS_SUCCESS) {
		...
		if (Metrics & BIT(3)) {
			...
			printf("FER period. Stable : %d ms, Degrading : %d ms, "
			       ...
			       le32_to_cpu(
				       metricscmd->UnacceptableFERPeriod_ms));
		}
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamGetScanPeriod(int argc, char *argv[])
{
	int ioctl_val, subioctl_val, cmd_len, cmdresplen;
	...
	for (state = STATE_STABLE - 1; state < STATE_UNACCEPTABLE; state++) {
		...
		for (scanmode = DISCOVERY_MODE - 1; scanmode < TRACKING_MODE;
		     scanmode++) {
			printf("%s = %6d ms\t", scanmodes[scanmode],
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int getControl(HostCmd_DS_CMD_ROAMAGENT_CONTROL *pcontrol)
{
	int ioctl_val, subioctl_val, cmd_len, cmdresplen;
	struct iwreq iwr;
	t_u8 *buf, *pos;
	...
	cmd_len = cmdresplen =
		sizeof(HostCmd_DS_GEN) + sizeof(roamcontrolcmd->action);
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamGetEventBitmap(int argc, char *argv[])
{
	t_u8 hostevent;
	HostCmd_DS_CMD_ROAMAGENT_CONTROL roamcontrolcmd;
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamGetControl(int argc, char *argv[])
{
	t_u8 control;
	HostCmd_DS_CMD_ROAMAGENT_CONTROL roamcontrolcmd;
 ...
 *  @return                   MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int getBackOff(HostCmd_DS_CMD_ROAMAGENT_BACKOFF *roambackoffcmdParam)
{
	int ioctl_val, subioctl_val, cmd_len, cmdresplen;
	struct iwreq iwr;
	t_u8 *buf, *pos;
	...
	cmd_len = cmdresplen =
		sizeof(HostCmd_DS_GEN) + sizeof(roambackoffcmd->action);
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamGetBackOff(int argc, char *argv[])
{
...
static int printGetNeighborDeprecation(int argc, char *argv[])
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetPreBeaconLoss(int argc, char *argv[])
{
	int ioctl_val, subioctl_val, cmd_len;
	int bcnmiss;
	struct iwreq iwr;
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetFailureCount(int argc, char *argv[])
{
	int ioctl_val, subioctl_val, cmd_len;
	int failCount, failTimeThresh;
	struct iwreq iwr;
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetStatsThreshold(int argc, char *argv[], int type)
{
	int ioctl_val, subioctl_val, cmd_len;
	...
	if (argv[0] && strncmp(argv[0], "stable", strlen("stable")) == 0) {
		...
		/* degrad[ing] */
		   ...
		   strncmp(argv[0], "degrad", strlen("degrad")) == 0) {
	...
	iwr.u.data.length =
		(sizeof(HostCmd_DS_GEN) + sizeof(statsthreshold->action) +
		 sizeof(statsthreshold->State) +
		 sizeof(statsthreshold->Profile) + i);
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetRssiStatsThreshold(int argc, char *argv[])
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetScanPeriod(int argc, char *argv[])
{
	int ioctl_val, subioctl_val, cmd_len;
	...
	scanPeriodInfo->scanPeriod.Header.type =
		cpu_to_le16(scanPeriodInfo->scanPeriod.Header.type);
	scanPeriodInfo->scanPeriod.Header.len =
		cpu_to_le16(scanPeriodInfo->scanPeriod.Header.len);
	scanPeriodInfo->scanPeriod.SearchMode =
		cpu_to_le16(scanPeriodInfo->scanPeriod.SearchMode);
	scanPeriodInfo->scanPeriod.State =
		cpu_to_le16(scanPeriodInfo->scanPeriod.State);
	scanPeriodInfo->scanPeriod.ScanPeriod =
		cpu_to_le32(scanPeriodInfo->scanPeriod.ScanPeriod);
 ...
 */
static inline int isdigitstr(char *str)
{
	unsigned int i = 0;
 ...
 */
static int process_subcmd(char *valid_cmds[], int count, int argc, char *argv[],
			  int *param, int *value)
{
	int ret = 0;
	int j = *param;
	...
	while (1) {
		for (k = 0; k < count; k++)
			if (argv[j] && !strncmp(valid_cmds[k], argv[j],
						strlen(valid_cmds[k])))
				break;
		if (k >= count) {
			break;
			...
			/* special case */
			if (!strncmp(valid_cmds[k], "perperiod",
				     strlen("perperiod")) ||
...
static int printSetNeighborDeprecation(int argc, char *argv[])
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetNborParams(int argc, char *argv[])
{
	...
	while (argc - j) {
		for (i = 0; (unsigned int)i < NELEMENTS(valid_cmds); i++) {
			if (!strncmp(valid_cmds[i], argv[j],
				     strlen(valid_cmds[i])))
				break;
		}
		...
		case 5: /** roamthresh */
			...
			for (i = 0; (i < 12); i++) {
				...
				switch (i % 3) {
				case 0:
					pNborAssessment->RoamThresh[i / 3]
						.RssiHighLevel = temp;
					break;
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetMetrics(int argc, char *argv[])
{
	int ioctl_val, subioctl_val, cmd_len, ret;
	HostCmd_DS_GEN *hostcmd;
	HostCmd_DS_CMD_ROAMAGENT_ADV_METRIC_THRESHOLD *metricscmd;
	...
	char *valid_fercmds[] = {"inactivity", "ferthreshold", "framethreshold",
				 "ferperiod"};
 ...
 *
 */
static int roamSetNborMaintenance(int argc, char *argv[])
{
	int ioctl_val, subioctl_val, cmd_len;
	struct iwreq iwr;
	t_u8 *buf, *pos;
	...
	case 0: /** blacklist */
		j++;
		sscanf(argv[j], "%2x:%2x:%2x:%2x:%2x:%2x", mac, mac + 1,
	...
	pNborMaintainance->action =
		((i == 0) ? HostCmd_ACT_ADD_TO_BLACKLIST :
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetBackOff(int argc, char *argv[])
{
	int ioctl_val, subioctl_val, cmd_len;
	struct iwreq iwr;
	t_u8 *buf, *pos, i = 0;
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetControl(int argc, char *argv[], int setreset, t_u8 value)
{
	int ioctl_val, subioctl_val, cmd_len;
	struct iwreq iwr;
	t_u8 *buf, *pos;
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetEnableControl(int argc, char *argv[])
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetDisableControl(int argc, char *argv[])
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetSuspendControl(int argc, char *argv[])
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetResumeControl(int argc, char *argv[])
{
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetCrossband(int argc, char *argv[])
{
	if (argc != 1) {
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int roamSetEventBitmap(int argc, char *argv[])
{
...
{
	sub_cmd_exec_t subCmds[] = {{"rssi", 2, 1, roamSetRssiStatsThreshold},
				    {"prebcnmiss", 2, 1, roamSetPreBeaconLoss},
				    {"failcnt", 2, 1, roamSetFailureCount},
				    {"neighbor", 2, 0,
				     printSetNeighborDeprecation},
