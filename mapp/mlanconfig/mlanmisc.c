...
int process_getscantable_idx(wlan_ioctl_get_scan_table_info *prsp_info_req)
{
	...
	printf("Channel = %d, SS = %d, CapInfo = 0x%04x, BcnIntvl = %d\n",
	       fixed_fields.channel, 255 - fixed_fields.rssi, tmp_cap,
	       beacon_interval);
...
int process_sdcmd52rw(int argc, char *argv[])
{
	...
	if (get_priv_ioctl("sdcmd52rw", &ioctl_val, &subioctl_val) ==
	    MLAN_STATUS_FAILURE) {
		return -EOPNOTSUPP;
	}
...
int process_sdcmd53rw(int argc, char *argv[])
{
	...
	if (get_priv_ioctl("sdcmd53rw", &ioctl_val, &subioctl_val) ==
	    MLAN_STATUS_FAILURE) {
		return -EOPNOTSUPP;
	}
...
int process_getscantable(int argc, char *argv[])
{
	...
	struct wlan_ioctl_get_scan_list *curr = NULL, *prev = NULL,
					*next = NULL;
	...
	do {
		...
		for (idx = 0; (unsigned int)idx < prsp_info->scan_number;
			...
			if (scan_list_head == NULL) {
				...
				curr_ss = 255 - scan_list_head->fixed_buf
							.fixed_fields.rssi;
	...
	for (curr = scan_list_head, idx = 0;
		...
		printf("%02u| %03d | %03d | %02x:%02x:%02x:%02x:%02x:%02x |",
		       idx, fixed_fields.channel, 255 - fixed_fields.rssi,
		       fixed_fields.bssid[0], fixed_fields.bssid[1],
		       fixed_fields.bssid[2], fixed_fields.bssid[3],
		       fixed_fields.bssid[4], fixed_fields.bssid[5]);
...
int process_setuserscan(int argc, char *argv[])
{
	...
	for (arg_idx = 0; arg_idx < argc; arg_idx++) {
		...
		} else if (strncmp(argv[arg_idx], "dur=", strlen("dur=")) ==
			   0) {
			/*
