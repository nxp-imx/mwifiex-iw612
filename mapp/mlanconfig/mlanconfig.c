...
static int we_version_compiled = 0; /**< version compiled */
 ...
 */
static int get_private_info(const t_s8 *ifname)
{
 ...
 */
static int nxp_get_subioctl_no(t_s32 i, t_s32 priv_cnt, int *ioctl_val,
			       int *subioctl_val)
{
	t_s32 j;
 ...
 *  @return 	        MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static int nxp_get_ioctl_no(const t_s8 *ifname, const t_s8 *priv_cmd,
...
int get_priv_ioctl(char *ioctl_name, int *ioctl_val, int *subioctl_val)
{
	...
	retval =
		nxp_get_ioctl_no(dev_name, ioctl_name, ioctl_val, subioctl_val);
 ...
 */
static int findcommand(t_s32 maxcmds, t_s8 *cmds[], t_s8 *cmd)
{
	t_s32 i;
...
static int process_arpfilter(int argc, char *argv[])
{
	...
	if (get_priv_ioctl("arpfilter", &ioctl_val, &subioctl_val) ==
	    MLAN_STATUS_FAILURE) {
		return -EOPNOTSUPP;
	}
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int process_cfg_data(int argc, char *argv[])
{
	t_u8 *buf;
	HostCmd_DS_GEN *hostcmd;
	struct iwreq iwr;
...
static int process_addts(int argc, char *argv[])
{
	...
	if (get_priv_ioctl("addts", &ioctl_val, &subioctl_val) ==
	    MLAN_STATUS_FAILURE) {
		return -EOPNOTSUPP;
	}
	...
	iwr.u.data.length = (sizeof(addtsReq) - sizeof(addtsReq.ieData) +
			     addtsReq.ieDataLen);
...
static int process_delts(int argc, char *argv[])
{
	...
	if (get_priv_ioctl("delts", &ioctl_val, &subioctl_val) ==
	    MLAN_STATUS_FAILURE) {
		return -EOPNOTSUPP;
	}
	...
	iwr.u.data.length = (sizeof(deltsReq) - sizeof(deltsReq.ieData) +
			     deltsReq.ieDataLen);
...
static int process_qconfig(int argc, char *argv[])
{
	...
	if (get_priv_ioctl("qconfig", &ioctl_val, &subioctl_val) ==
	    MLAN_STATUS_FAILURE) {
		return -EOPNOTSUPP;
	}
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int process_qstats(int argc, char *argv[])
{
	int ioctl_val, subioctl_val;
	struct iwreq iwr;
	...
	if (get_priv_ioctl("qstats", &ioctl_val, &subioctl_val) ==
	    MLAN_STATUS_FAILURE) {
		return -EOPNOTSUPP;
	}
	...
	if ((argc > 3) && strcmp(argv[3], "on") == 0) {
		 ...
		 * argument. The mlanconfig application invokes "get" option for
		 * all UPs
		 */
		if ((argc == 4) || (argc == 3)) {
			up_idx_start = 0;
...
static int process_wmm_qstatus(int argc, char *argv[])
{
	...
	if (get_priv_ioctl("qstatus", &ioctl_val, &subioctl_val) ==
	    MLAN_STATUS_FAILURE) {
		return -EOPNOTSUPP;
	}
...
static int process_wmm_ts_status(int argc, char *argv[])
{
	...
	if (get_priv_ioctl("ts_status", &ioctl_val, &subioctl_val) ==
	    MLAN_STATUS_FAILURE) {
		return -EOPNOTSUPP;
	}
 ...
 * @return         N/A
 **/
static int process_custom_ie(int argc, char *argv[])
{
 ...
 */
char *mlan_config_get_line(FILE *fp, t_s8 *str, t_s32 size, int *lineno)
{
	char *start, *end;
	int out, next_line;
 ...
 *  @return         MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int process_mef_cfg(int argc, char *argv[])
{
	int ioctl_val, subioctl_val;
 ...
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int process_mgmt_frame_tx(int argc, char *argv[])
{
	struct ifreq ifr;
	char *line = NULL;
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "PktSubType") == 0) {
			...
			if ((ret = mac2raw(mac_addr, peer_mac)) !=
			    MLAN_STATUS_SUCCESS) {
