 ...
 *    @return             Hex integer
 */
static unsigned char hexc2bin(char chr)
{
	if (chr >= '0' && chr <= '9')
		chr -= '0';
	else if (chr >= 'A' && chr <= 'F')
 ...
 *  @return         N/A
 */
static void print_event_sta_deauth(t_u8 *buffer, t_u16 size)
{
 ...
 *  @return         N/A
 */
static void print_event_wep_icv_error(t_u8 *buffer, t_u16 size)
{
	int i = 0;
 ...
 *  @return         N/A
 */
static void print_mgmt_frame(MrvlIETypes_MgmtFrameSet_t *mgmt_tlv, int tlv_len)
{
 ...
 *  @return         N/A
 */
static void print_event_rsn_connect(t_u8 *buffer, t_u16 size)
{
 ...
 *  @return         N/A
 */
static void print_event_sta_assoc(t_u8 *buffer, t_u16 size)
{
	int tlv_buf_left = size;
	t_u16 tlv_type, tlv_len;
	tlvbuf_header *tlv = NULL;
 ...
 *  @return         N/A
 */
static void print_event_bss_start(t_u8 *buffer, t_u16 size)
{
 ...
 *  @return         N/A
 */
static void print_event_mic_countermeasures(t_u8 *buffer, t_u16 size)
{
 ...
 *  @return         N/A
 */
static void print_wifi_wps_ie_elements(t_u8 *buffer, t_u16 size)
{
	t_u8 *ptr = buffer;
	t_u8 *array_ptr;
	int i;
 ...
 *  @return         N/A
 */
static void print_wifidirect_ie_elements(t_u8 *buffer, t_u16 size)
{
	t_u8 *ptr = buffer;
		...
		if (left_len > 0 && next_byte == WIFIDIRECT_OVERLAP_DATA) {
			...
			case TLV_TYPE_WIFIDIRECT_INVITATION_FLAG: {
				...
				printf("\t Invitation Flag = %d\n",
				       (int)wifidirect_tlv->invitation_flag &
				...
				if (isalpha(wifidirect_tlv->country_string[2]))
					printf("%c",
					       wifidirect_tlv
						       ->country_string[2]);
				printf("\n");
				temp_ptr = (chan_entry *)wifidirect_tlv
						   ->wifidirect_chan_entry_list;
				temp = uap_le16_to_cpu(wifidirect_tlv->length) -
			...
			case TLV_TYPE_WIFIDIRECT_DEVICE_INFO: {
				...
				printf("\t Primay device type = %02d-%02X%02X%02X%02X-%02d\n",
				       (int)ntohs(
					       wifidirect_tlv->primary_category),
				...
				for (i = 0;
				     i < wifidirect_tlv->secondary_dev_count;
				     i++) {
				...
				/* display device name */
				array_ptr =
					wifidirect_tlv->secondary_dev_info +
					(wifidirect_tlv->secondary_dev_count *
					 WPS_DEVICE_TYPE_LEN) +
					2 * sizeof(t_u16);
				if (*(t_u16 *)(((t_u8 *)(wifidirect_tlv
								 ->secondary_dev_info)) +
					       (wifidirect_tlv
							->secondary_dev_count *
						WPS_DEVICE_TYPE_LEN) +
			...
			case TLV_TYPE_WIFIDIRECT_GROUP_INFO: {
				...
				temp_ptr = (wifidirect_client_dev_info
						    *)wifidirect_tlv
 ...
 *  @return         N/A
 */
static void print_event_wifidirect_generic(t_u8 *buffer, t_u16 size)
{
	...
	while (wifidirect_wps_len >= sizeof(wifidirect_ie_header)) {
		if (!memcmp(wifidirect_wps_header->oui, wifi_oui,
			    sizeof(wifi_oui)) ||
		    !(memcmp(wifidirect_wps_header->oui, wps_oui,
			     sizeof(wps_oui)))) {
		...
		wifidirect_wps_header =
			(wifidirect_ie_header
				 *)(((t_u8 *)wifidirect_wps_header) +
				    wifidirect_wps_header->ie_length +
				    IE_HEADER_LEN);
	}
}
 ...
 *  @return         N/A
 */
static void print_event_wifidirect_service_discovery(t_u8 *buffer, t_u16 size)
{
	unsigned int i;
	t_u16 event_len = 0;
	...
	if (action == WIFIDIRECT_DISCOVERY_REQUEST_ACTION) {
		wifidirect_disc_req = (apeventbuf_wifidirect_discovery_request
					       *)(buffer + sizeof(t_u16));
		printf("\t Peer Mac Address - ");
		...
		printf("\t Service update Indicator = %d\n",
		       uap_le16_to_cpu(
			       wifidirect_disc_req->service_update_indicator));
		...
		if (wifidirect_disc_req->service_protocol == 1) {
			printf(" * Bonjour * \n");
			printf("\t\t DNS = ");
			dns_len = uap_le16_to_cpu(
					  wifidirect_disc_req->vendor_len) -
		...
		wifidirect_disc_resp = (apeventbuf_wifidirect_discovery_response
						*)(buffer + sizeof(t_u16));
		printf("\t Peer Mac Address - ");
		...
		printf("\t Service update Indicator = %d\n",
		       uap_le16_to_cpu(
			       wifidirect_disc_resp->service_update_indicator));
		printf("\t Vendor length = %d\n",
		...
		if (wifidirect_disc_resp->service_protocol == 1) {
			...
			*/
			...
			dns_len = uap_le16_to_cpu(
					  wifidirect_disc_resp->vendor_len) -
				  WIFIDIRECT_DISCOVERY_UPNP_FIXED_LEN;
			printf("\t\t Value = ");
			for (i = 0; i < dns_len; i++)
				printf("%02x ",
 ...
 *  @return         N/A
 */
static void print_event_tdls_generic(t_u8 *buffer, t_u16 size)
{
 ...
 *  @return         N/A
 */
static void print_reject_state(t_u8 state)
{
	switch (state) {
 ...
 *  @return         N/A
 */
static void print_reject_reason(t_u16 reason)
{
	switch (reason) {
 ...
 *  @return         N/A
 */
static void print_eapol_state(t_u8 state)
{
	switch (state) {
	case EAPOL_START:
 ...
 *  @return         N/A
 */
static void print_event_debug(t_u8 *buffer, t_u16 size)
{
	...
	if (uap_le32_to_cpu(event_body->debug_id_major) ==
		...
		case DEBUG_MAJ_AUTH_MIN_STA_REJ:
			...
			printf("\n");
			print_reject_state(
				event_body->info.sta_reject.reject_state);
		...
		case DEBUG_MAJ_AUTH_MIN_EAPOL_TR:
			...
			printf("\n");
			print_eapol_state(
				event_body->info.eapol_state.eapol_state);
			break;
		default:
			printf("ERR: unknow debug_id_minor: %d\n",
			       (int)uap_le32_to_cpu(
				       event_body->debug_id_minor));
			hexdump(buffer, size, ' ');
			return;
		}
 ...
 *  @return         N/A
 */
static void print_event_11k_nlist_report(t_u8 *buffer, t_u16 size)
{
	int tlv_buf_left = 0;
	tlvbuf_header *tlv;
...
#define CUS_EVT_TDLS_TEARDOWN "EVENT=TDLS_TEARDOWN"
		if (!strncmp((char *)event, CUS_EVT_TDLS_TEARDOWN,
...
#define CUS_EVT_STA_CONNECTED "EVENT=STA_CONNECTED"
		if (!strncmp((char *)event, CUS_EVT_STA_CONNECTED,
...
#define CUS_EVT_MLME_MIC_ERR_UNI "MLME-MICHAELMICFAILURE.indication unicast"
		if (!strncmp((char *)event, CUS_EVT_MLME_MIC_ERR_UNI,
...
#define CUS_EVT_MLME_MIC_ERR_MUL "MLME-MICHAELMICFAILURE.indication multicast"
		if (!strncmp((char *)event, CUS_EVT_MLME_MIC_ERR_MUL,
...
#define CUS_EVT_CAC_FINISHED "EVENT=CAC_FINISHED"
		if (!strncmp((char *)event, CUS_EVT_RADAR_DETECTED,
			     strlen(CUS_EVT_RADAR_DETECTED))) {
			printf("%s\n", (char *)event);
			break;
		}
		if (!strncmp((char *)event, CUS_EVT_CAC_FINISHED,
			     strlen(CUS_EVT_CAC_FINISHED))) {
			printf("%s\n", (char *)event);
			break;
		}
		if (!strncmp((char *)event, CUS_EVT_TOD_TOA,
			     strlen(CUS_EVT_TOD_TOA))) {
			printf("EVENT: MLAN ToD(Tx Pkt) and ToA(Ack Pkt)\n");
			printf("%s\n", (char *)event);
			break;
		}
		...
		if (!strncmp((char *)event, CUS_EVT_MLAN_CSI,
			...
			t_u16 csi_length =
				0x1fff &
				((event_mlan_csi *)event)->csi_record.Len;
			...
			printf("CSI record:\n");
			hexdump((void *)&(
					((event_mlan_csi *)event)->csi_record),
				4 * csi_length, ' ');

			/* Save CSI data to file */

			snprintf(filename, sizeof(filename), "csidump_%s.txt",
				 if_name);
			fstat = stat(filename, &fbuf);

			if ((fstat == 0) &&
			    (fbuf.st_size >= CSI_DUMP_FILE_MAX)) {
...
#define FW_DEBUG_INFO "EVENT=FW_DEBUG_INFO"
		if (!strncmp((char *)event, FW_DEBUG_INFO,
