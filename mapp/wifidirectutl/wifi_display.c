 ...
 *  $return         N/A
 */
static void print_wifidisplay_config_usage(void)
{
 ...
 */
static int wifiDisplay_ie_config(t_s16 *ie_index, t_u16 data_len_wifidisplay,
				 t_u8 *buf)
{
	struct iwreq iwr;
	t_s32 sockfd;
...
#define DISPLAY_MASK 0xFFFF
	...
	iwr.u.data.length = ((2 * sizeof(custom_ie)) +
			     sizeof(tlvbuf_custom_ie) + data_len_wifidisplay);
	iwr.u.data.flags = 0;
		...
		*ie_index = -1;
		if (tlv->tag == MRVL_MGMT_IE_LIST_TLV_ID) {
			...
			for (i = 0; i < MAX_MGMT_IE_INDEX; i++) {
				// Index 0 and 1 are reserved for WFD in current
				// implementation
...
void wifidisplaycmd_config(int argc, char *argv[])
{
	...
	if (argc >= 3) {
		...
		wifidisplay_file_params_config(argv[2], argv[1],
...
void wifidisplay_file_params_config(char *file_name, char *cmd_name, t_u8 *pbuf,
				    t_u16 *ie_len_wifidisplay)
{
	...
	t_u16 display_device_info = 0, session_mgmt_control_port = 0,
	      wfd_device_throuput = 0;
	...
	t_u8 dev_info_dev_add[ETH_ALEN] = {0},
	     dev_info_assoc_bssid[ETH_ALEN] = {0},
	     dev_info_coupled_add[ETH_ALEN];
	t_u16 descriptor_display_device_info = 0,
	      descriptor_wfd_device_throuput = 0;
	t_u8 descriptor_wfd_coupled_sink_status = 0;
	t_u8 wfd_dev_descriptor_arr[120], device_info_desc_len = 0,
					  ind = DEVICE_DESCRIPTOR_LEN;
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "display_dev_info") == 0) {
			...
			if (is_wifidisplay_input_valid(WFD_DEVICE_INFO,
						       arg_num - 1,
						       args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_wifidisplay_input_valid(WFD_DEVICE_THROUGHPUT,
						       arg_num - 1,
						       args + 1) != SUCCESS) {
				goto done;
			}
			...
			strncpy(wfd_mac, args[1], sizeof(wfd_mac) - 1);
			if ((ret = mac2raw(wfd_mac, alternate_mac)) !=
			    SUCCESS) {
				printf("ERR: %s Address \n",
				       ret == FAILURE ?
					       "Invalid MAC" :
			...
			if (is_wifidisplay_input_valid(WFD_COUPLED_SINK,
						       arg_num - 1,
						       args + 1) != SUCCESS) {
				goto done;
			}
			...
			strncpy(wfd_mac, args[1], sizeof(wfd_mac) - 1);
			if ((ret = mac2raw(wfd_mac, dev_info_dev_add)) !=
			    SUCCESS) {
				printf("ERR: %s Address \n",
				       ret == FAILURE ?
					       "Invalid MAC" :
		...
		} else if (strncmp(args[0], "device_info_assoc_bssid", 18) ==
			   0) {
			strncpy(wfd_mac, args[1], sizeof(wfd_mac) - 1);
			if ((ret = mac2raw(wfd_mac, dev_info_assoc_bssid)) !=
			    SUCCESS) {
				printf("ERR: %s Address \n",
				       ret == FAILURE ?
					       "Invalid MAC" :
		...
		} else if (strncmp(args[0], "descriptor_device_info", 22) ==
			   0) {
			...
			descriptor_wfd_coupled_sink_status =
				(t_u8)atoi(args[1]);
		} else if (strncmp(args[0], "device_info_coupled_address",
				   27) == 0) {
			strncpy(wfd_mac, args[1], sizeof(wfd_mac) - 1);
			if ((ret = mac2raw(wfd_mac, dev_info_coupled_add)) !=
			    SUCCESS) {
				printf("ERR: %s Address \n",
				       ret == FAILURE ?
					       "Invalid MAC" :
			...
			case DISPLAY_DEVICE_INFO: {
				...
				tlv->length = htons(tlv_len - (sizeof(t_u8) +
							       sizeof(t_u16)));
				*ie_len_wifidisplay = cmd_len_wifidisplay;
				...
				memcpy(&tlv->session_mgmt_control_port, &temp,
				       2);
			...
			case DISPLAY_ASSOCIATED_BSSID: {
				...
				if (memcmp(default_mac, assoc_bssid,
					   ETH_ALEN)) {
					/* Append a new TLV */
					tlv_len = sizeof(
						tlvbuf_wfdisplay_assoc_bssid);
					tlv = (tlvbuf_wfdisplay_assoc_bssid
						       *)(buffer +
							  cmd_len_wifidisplay);
					cmd_len_wifidisplay += tlv_len;
					*ie_len_wifidisplay =
					...
					memcpy(tlv->assoc_bssid, assoc_bssid,
					       ETH_ALEN);
			...
			case DISPLAY_SESSION_INFO: {
				...
				if (curr_dev_info > 0) {
					tlv_len = DEVICE_DESCRIPTOR_LEN *
							  curr_dev_info +
						  2;
					tlv = (tlvbuf_wifi_display_session_info
						       *)(buffer +
							  cmd_len_wifidisplay);
					cmd_len_wifidisplay += tlv_len;
					*ie_len_wifidisplay =
			...
			case DISPLAY_ALTERNATE_MAC_ADDR: {
				...
				if (memcmp(default_mac, alternate_mac,
					   ETH_ALEN)) {
					/* Append a new TLV */
					tlv_len = sizeof(
						tlvbuf_wfdisplay_alternate_mac);
					tlv = (tlvbuf_wfdisplay_alternate_mac
						       *)(buffer +
							  cmd_len_wifidisplay);
					cmd_len_wifidisplay += tlv_len;
					*ie_len_wifidisplay =
 ...
 *  $return         N/A
 */
static void print_wifidisplay_discovery_usage(void)
{
...
void wifidisplaycmd_service_discovery(int argc, char *argv[])
{
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "PeerAddr") == 0) {
			strncpy(wifidisplay_mac, args[1],
				sizeof(wifidisplay_mac) - 1);
			if ((ret = mac2raw(wifidisplay_mac, dev_address)) !=
			    SUCCESS) {
				printf("ERR: %s Address \n",
				       ret == FAILURE ?
					       "Invalid MAC" :
			...
			if (!req_resp) {
				...
				resp_buf = (wifidisplay_discovery_response *)
					buffer;
				for (i = 0; i < ie_len_wifidisplay; i++)
 ...
 *  $return         N/A
 */
static void print_wifi_display_status_usage(void)
{
...
int is_wifidisplay_input_valid(display_valid_inputs cmd, int argc, char *argv[])
{
	if (argc == 0)
		return FAILURE;
	if (argc != 1) {
...
void wifidisplay_update_custom_ie(int argc, char *argv[])
{
	...
	if (display_ie_index < (MAX_MGMT_IE_INDEX - 1)) {
		...
		if (tlv->tag == MRVL_MGMT_IE_LIST_TLV_ID) {
			...
			/* Update New IE now */
			wifiDisplay_ie_config(
				(t_s16 *)&display_ie_index, ie_len_wfd_org,
