 ...
 */
#ifdef DEBUG
static void hexdump(void *p, t_s32 len, char delim)
{
	t_s32 i;
	t_u8 *s = p;
...
static int wifidir_use_fixed_ie_indices(void)
{
#define WIFIDIR_USE_FIXED_IE_INDICES "WIFIDIR_USE_FIXED_IE_INDICES"
	const char *ret = getenv(WIFIDIR_USE_FIXED_IE_INDICES);
 ...
 */
static int string2raw(unsigned char *str, unsigned char *raw)
{
	int len = (strlen((const char *)str) + 1) / 2;
 ...
 */
static int ishexstring(void *hex)
{
	int i, a;
	char *p = hex;
	int len = strlen(p);
...
/**
 *  @brief Show usage information for the wifidirect_gas_comeback_discovery
 * commands
 *
 *  $return         N/A
 */
static void print_wifidirect_gas_comeback_usage(void)
{
...
/**
 *  @brief Creates a wifidirect_gas_comeback_service_discovery request/response
 ...
 *  @return         SUCCESS or FAILURE
 **/
static void wifidirectcmd_gas_comeback_discovery(int argc, char *argv[])
{
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "wifidirect_gas_comeback_request") == 0) {
			...
			cmd_found = 1;
				  ...
				  "wifidirect_gas_comeback_response") == 0) {
			wifidirect_level =
				WIFIDIRECT_DISCOVERY_REQUEST_RESPONSE;
			req_resp = 1;
		...
		if (strcmp(args[0], "PeerAddr") == 0) {
			...
			if ((ret = mac2raw(wifidirect_mac, dev_address)) !=
			    SUCCESS) {
				printf("ERR: %s Address \n",
				       ret == FAILURE ?
					       "Invalid MAC" :
					       ret == WIFIDIRECT_RET_MAC_BROADCAST ?
					       "Broadcast" :
					       "Multicast");
				goto done;
			}
			(!req_resp) ? memcpy(req_buf->peer_mac_addr,
				      ...
				      memcpy(resp_buf->peer_mac_addr,
					     dev_address, ETH_ALEN);

		} else if (strcmp(args[0], "Category") == 0) {
			if (is_input_valid(WIFIDIRECT_CATEGORY, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			(!req_resp) ?
				(req_buf->category = (t_u8)atoi(args[1])) :
				(resp_buf->category = (t_u8)atoi(args[1]));
		} else if (strcmp(args[0], "Action") == 0) {
			if (is_input_valid(WIFIDIRECT_ACTION, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_DIALOGTOKEN, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			(!req_resp) ?
				(req_buf->dialog_taken = (t_u8)atoi(args[1])) :
			...
			if (is_input_valid(WIFIDIRECT_GAS_COMEBACK_DELAY,
			...
			if (is_input_valid(WIFIDIRECT_DISC_ADPROTOIE,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_DISC_INFOID, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			for (i = 0; i < arg_num - 1; i++) {
			...
			if (is_input_valid(WIFIDIRECT_OUI, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			for (i = 0; i < arg_num - 1; i++) {
			...
			if (is_input_valid(WIFIDIRECT_OUISUBTYPE, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			if (req_resp)
			...
			if (is_input_valid(WIFIDIRECT_DISC_SERVICEPROTO,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			if (req_resp)
			...
			query_len++;
			/*
			 * For uPnP, due to union allocation, a extra byte is
			 * allocated reduce it here for uPnP
			 */
			cmd_len--;
		} else if (strcmp(args[0], "ServiceUpdateIndicator") == 0) {
			if (is_input_valid(WIFIDIRECT_SERVICEUPDATE_INDICATOR,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			if (req_resp)
			...
			if (is_input_valid(WIFIDIRECT_DISC_SERVICETRANSACID,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			if (req_resp)
			...
			if (is_input_valid(WIFIDIRECT_DISC_SERVICE_STATUS,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			if (req_resp)
			...
			if (is_input_valid(WIFIDIRECT_DISC_DNSTYPE, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			if (req_resp)
			...
			if (is_input_valid(WIFIDIRECT_DISC_BONJOUR_VERSION,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			if (req_resp)
			...
			if (is_input_valid(WIFIDIRECT_DISC_UPNP_VERSION,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			if (req_resp)
 ...
 *  $return         N/A
 */
static void print_wifidirect_discovery_usage(void)
{
 ...
 *  @return         SUCCESS or FAILURE
 **/
static void wifidirectcmd_service_discovery(int argc, char *argv[])
{
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "PeerAddr") == 0) {
			...
			(!req_resp) ?
				(req_buf->oui_sub_type = (t_u8)atoi(args[1])) :
				(resp_buf->oui_sub_type = (t_u8)atoi(args[1]));
			service_len++;
			query_len++;
		} else if (strcmp(args[0], "QueryRequestLen") == 0 ||
			...
			if (!req_resp) {
				...
				/*
				 * For uPnP, due to union allocation, a extra
				 * byte is allocated reduce it here for uPnP
				 */
			...
			if (args[1][0] == '"') {
				...
				(!req_resp) ?
					(strncpy((char *)req_buf->disc_query.u
							 .bonjour.dns,
						 args[1], strlen(args[1]))) :
					(strncpy((char *)resp_buf->disc_resp.u
							 .bonjour.dns,
			...
			(!req_resp) ? (req_buf->disc_query.u.upnp.version =
					       (t_u8)A2HEXDECIMAL(args[1])) :
				      (resp_buf->disc_resp.u.upnp.version =
					       (t_u8)A2HEXDECIMAL(args[1]));
			vendor_len++;
 ...
 *  $return         N/A
 */
static void print_wifidirect_config_usage(void)
{
...
static void wifidirect_file_params_config(char *file_name, char *cmd_name,
					  t_u8 *pbuf, t_u16 *ie_len_wifidirect,
					  t_u16 *ie_len_wps)
{
	FILE *config_file = NULL;
	char *line = NULL;
	int i = 0, li = 0, arg_num = 0, ret = 0, wifidirect_level = 0,
	    no_of_chan_entries = 0, no_of_noa = 0;
	...
	t_u16 sec_category = 0, sec_sub_category = 0,
	      group_sec_sub_category = 0, group_sec_category = 0;
	...
	t_u8 dev_capability = 0, group_capability = 0, cmd_found = 0,
	     ...
	     op_regulatory_class = 0, op_channel_number = 0,
	     invitation_flag = 0;
	t_u8 WPS_version = 0, WPS_setupstate = 0, WPS_requesttype = 0,
	     WPS_responsetype = 0, WPS_UUID[WPS_UUID_MAX_LEN],
	     WPS_primarydevicetype[WPS_DEVICE_TYPE_MAX_LEN], WPS_RFband = 0,
	     WPS_modelnumber[32], WPS_serialnumber[32], WPS_VendorExt[32];
	...
	t_u8 wifidirect_client_dev_count = 0, wifidirect_client_dev_index = 0,
	     temp8 = 0;
	...
	t_u8 wifidirect_group_total_ssid_len = 0, tlv_offset = 0,
	     temp_dev_size = 0;
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "wifidirect_config") == 0) {
		...
		} else if (strncmp(args[0], "GroupWifiDirectDeviceAddress",
				   21) == 0) {
			strncpy(wifidirect_mac, args[1], 20 - 1);
			if ((ret = mac2raw(wifidirect_mac,
			...
			memcpy(wifidirect_client_dev_info_list
			   ...
			   0) {
			if (is_input_valid(WIFIDIRECT_DEVICECAPABILITY,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			memcpy(&wifidirect_client_dev_info_list
					       [wifidirect_client_dev_index - 1]
						       .wifidirect_dev_capability +
				       tlv_offset,
			       &temp8, sizeof(temp8));
		} else if (strncmp(args[0], "GroupWifiDirectWPSConfigMethods",
				   24) == 0) {
			if (is_input_valid(WIFIDIRECT_WPSCONFMETHODS,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			memcpy(&wifidirect_client_dev_info_list
					       [wifidirect_client_dev_index - 1]
						       .config_methods +
				       tlv_offset,
			       &temp16, sizeof(temp16));
			(t_u16) A2HEXDECIMAL(args[1]);
		} else if (strncmp(args[0], "GroupPrimaryDeviceTypeCategory",
				   30) == 0) {
			if (is_input_valid(WIFIDIRECT_PRIDEVTYPECATEGORY,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			memcpy(&wifidirect_client_dev_info_list
			   ...
			   0) {
			if (is_input_valid(WIFIDIRECT_PRIDEVTYPEOUI,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			for (i = 0; i < 4; i++) {
		...
		} else if (strncmp(args[0], "GroupPrimaryDeviceTypeSubCategory",
				   33) == 0) {
			if (is_input_valid(WIFIDIRECT_PRIDEVTYPESUBCATEGORY,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			memcpy(&wifidirect_client_dev_info_list
			   ...
			   0) {
			if (is_input_valid(WIFIDIRECT_GROUP_SECONDARYDEVCOUNT,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
		...
		} else if (strncmp(args[0], "GroupSecondaryDeviceTypeCategory",
				   30) == 0) {
			if (is_input_valid(WIFIDIRECT_PRIDEVTYPECATEGORY,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
		...
		} else if (strncmp(args[0], "GroupSecondaryDeviceTypeOUI",
				   27) == 0) {
			if (is_input_valid(WIFIDIRECT_PRIDEVTYPEOUI,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			...
			for (i = 0; i < 4; i++)
				group_secondary_oui[i] =
					(t_u8)A2HEXDECIMAL(args[i + 1]);
		} else if (strncmp(args[0],
				   "GroupSecondaryDeviceTypeSubCategory",
				   35) == 0) {
			if (is_input_valid(WIFIDIRECT_PRIDEVTYPESUBCATEGORY,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			...
			if (group_secondary_dev_count) {
			   ...
			   0) {
			if (is_input_valid(WIFIDIRECT_SECONDARYDEVCOUNT,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			memcpy(((t_u8 *)&wifidirect_client_dev_info_list
					[wifidirect_client_dev_index - 1]
						.wifidirect_device_name_type +
				tlv_offset),
			       &temp, sizeof(temp));
			...
			memcpy(((t_u8 *)&wifidirect_client_dev_info_list
				...
				tlv_offset),
			       &temp, sizeof(temp));
			memset(((t_u8 *)&wifidirect_client_dev_info_list
					[wifidirect_client_dev_index - 1]
						.wifidirect_device_name +
				tlv_offset),
			       0, strlen(wifi_group_direct_ssid));
			memcpy(((t_u8 *)&wifidirect_client_dev_info_list
					[wifidirect_client_dev_index - 1]
						.wifidirect_device_name +
				tlv_offset),
			...
			if (is_input_valid(WIFIDIRECT_GROUPCAPABILITY,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			/* Intent -> 0 - 15 */
			if (is_input_valid(WIFIDIRECT_INTENT, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			group_owner_intent = (t_u8)atoi(args[1]);
		} else if (strcmp(args[0], "RegulatoryClass") == 0) {
			if (is_input_valid(WIFIDIRECT_REGULATORYCLASS,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_NO_OF_CHANNELS,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			chan_buf += sizeof(t_u8) + sizeof(t_u8) +
				    chan_entry_num_of_channels;
		} else if (strcmp(args[0], "NoA_Index") == 0) {
			if (is_input_valid(WIFIDIRECT_NOA_INDEX, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			noa_index = (t_u8)atoi(args[1]);
		} else if (strcmp(args[0], "OppPS") == 0) {
			if (is_input_valid(WIFIDIRECT_OPP_PS, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			opp_ps = (t_u8)atoi(args[1]);
		} else if (strcmp(args[0], "CTWindow") == 0) {
			if (is_input_valid(WIFIDIRECT_CTWINDOW, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_COUNT_TYPE, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			no_of_noa++;
			...
			if (is_input_valid(WIFIDIRECT_DURATION, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_INTERVAL, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_START_TIME, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			...
			for (i = 0; i < 4; i++)
				primary_oui[i] =
					(t_u8)A2HEXDECIMAL(args[i + 1]);
		} else if (strcmp(args[0], "PrimaryDeviceTypeSubCategory") ==
			   0) {
			if (is_input_valid(WIFIDIRECT_PRIDEVTYPESUBCATEGORY,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			...
			secondary_index++;
		} else if (strncmp(args[0], "SecondaryDeviceTypeOUI", 22) ==
			   0) {
			if (is_input_valid(WIFIDIRECT_PRIDEVTYPEOUI,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			...
			for (i = 0; i < 4; i++)
				secondary_oui[i] =
					(t_u8)A2HEXDECIMAL(args[i + 1]);
		} else if (strncmp(args[0], "SecondaryDeviceTypeSubCategory",
				   30) == 0) {
			if (is_input_valid(WIFIDIRECT_PRIDEVTYPESUBCATEGORY,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			...
			if (secondary_dev_count) {
				memcpy(&secondary_dev_info[secondary_index *
							   WPS_DEVICE_TYPE_LEN],
			...
			if (is_input_valid(WIFIDIRECT_INTERFACECOUNT,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_ATTR_CONFIG_TIMEOUT,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_ATTR_EXTENDED_TIME,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_WPSDEVICENAME,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_WPSVERSION, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			WPS_version = (t_u8)A2HEXDECIMAL(args[1]);
		} else if (strcmp(args[0], "WPSSetupState") == 0) {
			if (is_input_valid(WIFIDIRECT_WPSSETUPSTATE,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_WPSREQRESPTYPE,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_WPSSPECCONFMETHODS,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			WPS_specconfigmethods = (t_u16)A2HEXDECIMAL(args[1]);
		} else if (strcmp(args[0], "WPSUUID") == 0) {
			if (is_input_valid(WIFIDIRECT_WPSUUID, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_WPSPRIMARYDEVICETYPE,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_WPSMODELNAME, arg_num - 1,
					   args + 1) != SUCCESS) {
				goto done;
			}
			...
			if (is_input_valid(WIFIDIRECT_WPSMODELNUMBER,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			for (i = 0; i < arg_num - 1; i++)
			...
			if (is_input_valid(WIFIDIRECT_WPSSERIALNUMBER,
					   arg_num - 1, args + 1) != SUCCESS) {
				goto done;
			}
			for (i = 0; i < arg_num - 1; i++)
			...
			case WIFIDIRECT_GROUP_OWNER_INTENT_CONFIG: {
				...
				tlv = (tlvbuf_wifidirect_group_owner_intent
					       *)(buffer + cmd_len_wifidirect);
				cmd_len_wifidirect += tlv_len;
				/* Set TLV fields */
				tlv->tag =
			...
			case WIFIDIRECT_NOTICE_OF_ABSENCE: {
				...
				tlv = (tlvbuf_wifidirect_notice_of_absence
					       *)(buffer + cmd_len_wifidirect);
				cmd_len_wifidirect += tlv_len;
				/* Set TLV fields */
				tlv->tag =
			...
			case WIFIDIRECT_DEVICE_INFO_CONFIG: {
				 ...
				 * already htons'ed */
				if (secondary_dev_count > 0)
					memcpy(tlv->secondary_dev_info,
					       secondary_dev_info,
					       secondary_dev_count *
						       WPS_DEVICE_TYPE_LEN);
				ptr = tlv->secondary_dev_info +
				      (secondary_dev_count *
				       WPS_DEVICE_TYPE_LEN);
				temp = htons(SC_Device_Name);
				memcpy(ptr, &temp, sizeof(temp));
				/* move by size of t_u16 device_name_type */
				ptr += sizeof(t_u16);
				temp = htons(strlen(wifidirect_ssid));
				memcpy(ptr, &temp, sizeof(temp));
				/* move by size of t_u16 device_name_len */
				ptr += sizeof(t_u16);
				memcpy(ptr, wifidirect_ssid,
			...
			case WIFIDIRECT_GROUP_INFO_CONFIG: {
				...
				tlv_offset =
					...
					wifidirect_total_secondary_dev_count *
						WPS_DEVICE_TYPE_LEN;
				tlv_len =
					sizeof(tlvbuf_wifidirect_group_info) +
					wifidirect_client_dev_count *
						sizeof(wifidirect_client_dev_info) +
					tlv_offset;
			...
			case WIFIDIRECT_GROUP_ID_CONFIG: {
				...
				/* Append a new TLV */
				tlv_len = sizeof(tlvbuf_wifidirect_group_id) +
					  strlen(wifidirect_ssid);
			...
			case WIFIDIRECT_WPSIE: {
				...
				if (!wifidir_use_fixed_ie_indices()) {
					...
					wps_ie_buf =
						(special_mask_custom_ie_buf *)
							tlv;
					memcpy(&wps_ie_buf->Oui[0], wps_oui,
					       sizeof(wps_oui));
				...
				*(tlv->data) = WPS_RFband;
				...
				tlv_len =
					sizeof(tlvbuf_wps_ie) + wps_vendor_len;
				tlv = (tlvbuf_wps_ie *)(buffer +
							cmd_len_wifidirect +
							sizeof(custom_ie) +
							cmd_len_wps);
 ...
 */
static int wifidirect_ie_config(t_u16 *ie_index, t_u16 data_len_wifidirect,
		...
		*ie_index = 0xFFFF;
		if (tlv->tag == MRVL_MGMT_IE_LIST_TLV_ID) {
			...
			for (i = 0; i < MAX_MGMT_IE_INDEX; i++) {
				/* zero mask indicates a wps IE, return previous
				 * index */
 ...
 *  @return         None
 */
static void wifidirectcmd_config(int argc, char *argv[])
{
	...
	if (argc >= 3) {
		...
		if (ret == SUCCESS && ie_index < 3) {
			...
			if (tlv->tag == MRVL_MGMT_IE_LIST_TLV_ID) {
				...
				if (!wifidir_use_fixed_ie_indices()) {
					while (memcmp(ptr, wifidirect_oui,
						      sizeof(wifidirect_oui))) {
						ie_ptr =
							(custom_ie
								 *)((t_u8 *)ie_ptr +
				...
				while (ie_len_wifidirect >
					...
					case TLV_TYPE_WIFIDIRECT_CHANNEL_LIST: {
						...
						printf("\t Country String \"%c%c\"",
						       ...
						       wifidirect_tlv
							       ->country_string
								       [1]);
						if (isalpha(wifidirect_tlv
								    ->country_string
									    [2]))
							printf("\"%c\"",
							       wifidirect_tlv->country_string
								       [2]);
						printf("\n");
						temp_ptr =
							(chan_entry
					...
					case TLV_TYPE_WIFIDIRECT_GROUP_INFO: {
						...
						while (temp && temp_ptr) {
							...
							print_mac(
								temp_ptr->wifidirect_dev_address);
							printf("\n");
							printf("\t Group WifiDirect Client Interface address - ");
							print_mac(
								temp_ptr->wifidirect_intf_address);
							printf("\n");
							...
							printf("\t Group WifiDirect Client Primay device type = %02d-%02X%02X%02X%02X-%02d\n",
							       (int)ntohs(
								       temp_ptr->primary_category),
							       (int)temp_ptr->primary_oui
								       [0],
							       (int)temp_ptr->primary_oui
								       [1],
							       (int)temp_ptr->primary_oui
							...
							for (i = 0;
								...
								memcpy(&temp,
								       array_ptr,
								       sizeof(t_u16));
								printf("\t Group WifiDirect Client Secondary device type = %02d-",
								       ntohs(temp));
								array_ptr += sizeof(
									temp);
								printf("%02X%02X%02X%02X",
							 ...
							 */
							array_ptr =
								temp_ptr->wifidirect_device_name +
								temp_ptr->wifidirect_secondary_dev_count *
									WPS_DEVICE_TYPE_LEN;
							printf("\t Group WifiDirect Device Name =  ");
							memcpy(&temp,
							       (((t_u8 *)(&temp_ptr->wifidirect_device_name_len)) +
								temp_ptr->wifidirect_secondary_dev_count *
									WPS_DEVICE_TYPE_LEN),
							       sizeof(t_u16));
							temp = ntohs(temp);
					...
					case TLV_TYPE_WIFIDIRECT_INTERFACE: {
						...
						for (i = 0;
							...
							printf("\n\t Interface address [%d]",
							       i + 1);
							print_mac(
								&wifidirect_tlv->interface_idlist
									 [i *
									  ETH_ALEN]);
						}
						printf("\n");
				...
				while (ie_len_wps > sizeof(tlvbuf_wps_ie)) {
					...
					case SC_Config_Methods: {
						...
						tlvbuf_wps_ie *wps_tlv =
							(tlvbuf_wps_ie *)ptr;
						memcpy(&wps_config_methods,
						       wps_tlv->data,
						       sizeof(t_u16));
						wps_config_methods = ntohs(
							wps_config_methods);
					...
					case SC_Configuration_Error: {
						t_u16 wps_configuration_error =
							0;
						tlvbuf_wps_ie *wps_tlv =
							(tlvbuf_wps_ie *)ptr;
						memcpy(&wps_configuration_error,
						       wps_tlv->data,
						       sizeof(t_u16));
						wps_configuration_error = ntohs(
							wps_configuration_error);
						printf("\t WPS Configuration Error = 0x%x\n",
						       wps_configuration_error);
					} break;
					case SC_Device_Password_ID: {
						t_u16 wps_device_password_id =
							0;
						tlvbuf_wps_ie *wps_tlv =
							(tlvbuf_wps_ie *)ptr;
						memcpy(&wps_device_password_id,
						       wps_tlv->data,
						       sizeof(t_u16));
						wps_device_password_id = ntohs(
 ...
 *  $return         N/A
 */
static void print_wifidirect_params_config_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirectcmd_params_config(int argc, char *argv[])
{
	...
	t_u16 device_state = 0, tlv_len = 0, max_disc_int = 0, min_disc_int = 0,
	      cmd_len = 0;
	...
	if (argc >= 3) {
		...
		while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li,
			...
			if (strcmp(args[0], "EnableScan") == 0) {
				if (is_input_valid(WIFIDIRECT_ENABLE_SCAN,
						   arg_num - 1,
						   args + 1) != SUCCESS) {
					goto done;
				}
			...
			} else if (strcmp(args[0], "MinDiscoveryInterval") ==
				   0) {
				if (is_input_valid(WIFIDIRECT_MINDISCOVERYINT,
						   arg_num - 1,
						   args + 1) != SUCCESS) {
					goto done;
				}
				min_disc_int = cpu_to_le16(atoi(args[1]));
			} else if (strcmp(args[0], "MaxDiscoveryInterval") ==
				   0) {
				if (is_input_valid(WIFIDIRECT_MAXDISCOVERYINT,
						   arg_num - 1,
						   args + 1) != SUCCESS) {
					goto done;
				}
		...
		if (param_buf->result == CMD_SUCCESS) {
			...
			while (tlv_len > sizeof(tlvbuf_wps_ie)) {
				...
				case MRVL_WIFIDIRECT_PEER_DEVICE_TLV_ID: {
					memcpy(dev_address, new_tlv->data,
					       ETH_ALEN);
				...
				case MRVL_WIFIDIRECT_SCAN_REQ_DEVICE_TLV_ID: {
					...
					printf("\tType: %02x:%02x\n",
					       ...
					       (unsigned int)
						       WPS_primarydevicetype[1]);
					printf("\tOUI: %02x:%02x:%02x:%02x\n",
					       (unsigned int)
						       WPS_primarydevicetype[2],
					       (unsigned int)
						       WPS_primarydevicetype[3],
					       (unsigned int)
						       WPS_primarydevicetype[4],
					       (unsigned int)
						       WPS_primarydevicetype[5]);
					printf("\tSubType: %02x:%02x\n",
					       (unsigned int)
						       WPS_primarydevicetype[6],
					       (unsigned int)
						       WPS_primarydevicetype[7]);
				} break;
				case MRVL_WIFIDIRECT_DEVICE_STATE_TLV_ID: {
					memcpy(&device_state, &new_tlv->data,
					       sizeof(t_u16));
 ...
 *  $return         N/A
 */
static void print_wifidirect_action_frame_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirectcmd_action_frame(int argc, char *argv[])
{
	...
	if (config_file && argc == 3) {
		...
		while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li,
			...
			if (strcmp(args[0], "}") == 0) {
				...
				if (is_input_valid(WIFIDIRECT_CATEGORY,
						   arg_num - 1,
						   args + 1) != SUCCESS) {
					goto done;
				}
				...
				if (is_input_valid(WIFIDIRECT_ACTION,
						   arg_num - 1,
						   args + 1) != SUCCESS) {
					goto done;
				}
				...
				if (is_input_valid(WIFIDIRECT_DIALOGTOKEN,
						   arg_num - 1,
						   args + 1) != SUCCESS) {
					goto done;
				}
				...
				if (is_input_valid(WIFIDIRECT_OUITYPE,
						   arg_num - 1,
						   args + 1) != SUCCESS) {
					goto done;
				}
				...
				if (is_input_valid(WIFIDIRECT_OUISUBTYPE,
						   arg_num - 1,
						   args + 1) != SUCCESS) {
					goto done;
				}
		...
		wifidirect_file_params_config(
			argv[2], argv[1], action_buf->ie_list, &ie_len, NULL);
 ...
 *  $return         N/A
 */
static void print_wifidirect_status_usage(void)
{
 ...
 *  @return         N/A
 */
static void wifidirectcmd_status(int argc, char *argv[])
{
	int opt, ret;
	t_u16 data = 0;
	t_u16 cmd_len = 0;
 ...
 *  @return         N/A
 */
static void print_wifidirect_cfg_discovery_period_usage(void)
{
 ...
 *
 *  Usage: "Usage : wifidirect_cfg_discovery_period [<MinDiscPeriod>
 * <MaxDiscPeriod>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void wifidirect_cfg_cmd_discovery_period(int argc, char *argv[])
{
	...
	if ((argc != 2) && (argc != 4)) {
		...
		print_wifidirect_cfg_discovery_period_usage();
		return;
	} else if ((argc > 2) && ((is_input_valid(WIFIDIRECT_MINDISCOVERYINT, 1,
						  &argv[2]) != SUCCESS) ||
				  (is_input_valid(WIFIDIRECT_MAXDISCOVERYINT, 1,
						  &argv[3]) != SUCCESS))) {
		print_wifidirect_cfg_discovery_period_usage();
		return;
	}
	...
	tlv = (tlvbuf_wifidirect_discovery_period
		       *)(buffer + sizeof(wifidirect_params_config));
	...
	tlv->length = sizeof(tlvbuf_wifidirect_discovery_period) -
	...
	if (ret == SUCCESS) {
		/* Verify response */
		if (cmd_buf->cmd_code != (HostCmd_CMD_WIFIDIRECT_PARAMS_CONFIG |
					  WIFIDIRECTCMD_RESP_CHECK)) {
			printf("ERR:Corrupted response!\n");
			goto done;
		}
 ...
 *  @return         N/A
 */
static void print_wifidirect_cfg_intent_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirect_cfg_cmd_intent(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	if (argc < 2) {
		...
		print_wifidirect_cfg_intent_usage();
		return;
	} else if ((argc > 2) && (is_input_valid(WIFIDIRECT_INTENT, argc - 2,
						 argv + 2) != SUCCESS)) {
		print_wifidirect_cfg_intent_usage();
		return;
	}
 ...
 *  @return         N/A
 */
static void print_wifidirect_cfg_listen_channel_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirect_cfg_cmd_listen_channel(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	cmd_buf = (wifidirect_params_config *)buffer;
	tlv = (tlvbuf_wifidirect_listen_channel
		       *)(buffer + sizeof(wifidirect_params_config));
 ...
 *  @return         N/A
 */
static void print_wifidirect_cfg_op_channel_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirect_cfg_cmd_op_channel(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	cmd_buf = (wifidirect_params_config *)buffer;
	tlv = (tlvbuf_wifidirect_operating_channel
		       *)(buffer + sizeof(wifidirect_params_config));
	...
	tlv->length = sizeof(tlvbuf_wifidirect_operating_channel) -
		      MRVL_TLV_HEADER_SIZE;
 ...
 *  @return         N/A
 */
static void print_wifidirect_cfg_invitation_list_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirect_cfg_cmd_invitation_list(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	cmd_buf = (wifidirect_params_config *)buffer;
	tlv = (tlvbuf_wifidirect_invitation_list
		       *)(buffer + sizeof(wifidirect_params_config));
	...
	tlv->length = sizeof(tlvbuf_wifidirect_invitation_list) -
		      MRVL_TLV_HEADER_SIZE;
 ...
 *  @return         N/A
 */
static void print_wifidirect_cfg_noa_usage(void)
{
 ...
 *
 *  Usage: "Usage : wifidirect_cfg_noa <enable|disable> <index> [<counttype>
 * <duration> <interval>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void wifidirect_cfg_cmd_noa(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	if (argc > 2) {
		if (argc == 4) {
			...
			if (atoi(argv[3]) > WIFIDIRECT_NOA_DESC_MAX ||
			    (is_input_valid(WIFIDIRECT_COUNT_TYPE, 1,
					    &argv[4]) != SUCCESS) ||
	...
	if (argc == 2) {
		cmd_buf->action = ACTION_GET;
		cmd_len += (sizeof(tlvbuf_wifidirect_noa_config) -
			    MRVL_TLV_HEADER_SIZE) *
 ...
 *  @return         N/A
 */
static void print_wifidirect_cfg_opp_ps_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirect_cfg_cmd_opp_ps(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	if (argc == 4) {
		...
		if (is_input_valid(WIFIDIRECT_CTWINDOW, 1, &argv[3]) !=
		    SUCCESS) {
			print_wifidirect_cfg_opp_ps_usage();
			return;
		}
	...
	cmd_buf = (wifidirect_params_config *)buffer;
	tlv = (tlvbuf_wifidirect_opp_ps_config
		       *)(buffer + sizeof(wifidirect_params_config));
 ...
 *  @return         N/A
 */
static void print_wifidirect_cfg_capability_usage(void)
{
 ...
 *
 *  Usage: "Usage : wifidirect_cfg_capability [<DeviceCapability>
 * <GroupCapability>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void wifidirect_cfg_cmd_capability(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	cmd_buf = (wifidirect_params_config *)buffer;
	tlv = (tlvbuf_wifidirect_capability_config
		       *)(buffer + sizeof(wifidirect_params_config));
	...
	tlv->length = sizeof(tlvbuf_wifidirect_capability_config) -
		      MRVL_TLV_HEADER_SIZE;
...
/**
 *  @brief Show usage information for the wifidirect_cfg_persistent_group_record
 * command
 *
 *  $return         N/A
 */
static void print_wifidirect_peristent_group_usage(void)
{
 ...
 *  @return         SUCCESS or FAILURE
 **/
static void wifidirect_cfg_cmd_persistent_group_record(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
		...
		if (!strncasecmp("0x", argv[7], 2)) {
			...
			if (strlen(argv[7]) / 2 != WIFIDIRECT_PSK_LEN_MAX) {
				printf("ERR:Incorrect PSK length %d. It should be %d.\n",
				       (t_u32)strlen(argv[7]) / 2,
				       WIFIDIRECT_PSK_LEN_MAX);
				goto done;
			}
			string2raw((t_u8 *)argv[7],
				   tlv->group_ssid + var_len + sizeof(t_u8));
			*(tlv->group_ssid + var_len) = WIFIDIRECT_PSK_LEN_MAX;
			...
			memcpy(tlv->group_ssid + var_len + sizeof(t_u8),
			       argv[7], psk_len);
			*(tlv->group_ssid + var_len) = psk_len;
			var_len += psk_len;
		}
		var_len += sizeof(t_u8);

		tlv->length = sizeof(tlvbuf_wifidirect_persistent_group) -
			      MRVL_TLV_HEADER_SIZE + var_len;
		*(tlv->group_ssid + var_len) = 0;

		if (argc == 9 || argc == 10) {
			if ((ret = mac2raw(argv[8], tlv->group_ssid + var_len +
							    sizeof(t_u8))) !=
			    SUCCESS) {
				printf("ERR:Invalid MAC address %s\n", argv[8]);
				goto done;
			}
			*(tlv->group_ssid + var_len) = 1;
		}
		if (argc == 10) {
			if ((ret = mac2raw(argv[9], tlv->group_ssid + var_len +
							    sizeof(t_u8) +
							    ETH_ALEN)) !=
			    SUCCESS) {
				printf("ERR:Invalid MAC address %s\n", argv[8]);
			...
			*(tlv->group_ssid + var_len) = 2;
		}
		num_peers = *(tlv->group_ssid + var_len);

		/* sizeof(t_u8) for num_peers*/
		tlv->length += (num_peers * ETH_ALEN) + sizeof(t_u8);
		cmd_len += MRVL_TLV_HEADER_SIZE + tlv->length;
		buf_len = cmd_len;
		cmd_buf->size = cmd_len - BUF_HEADER_SIZE;
		break;
	...
	if (argc == 2 || argc == 3) {
		...
		buf_len = tlv->length;
		while (buf_len > (sizeof(tlvbuf_wifidirect_persistent_group) -
				  2 * ETH_ALEN)) {
			printf("\n\t Index = [%d]\n", tlv->rec_index);
			if (tlv->dev_role == 1)
			...
			var_len = tlv->group_ssid_len;
			printf("\n\t PSK = ");
			for (index = 0; index < *(tlv->group_ssid + var_len);
			     index++) {
				if (index == 16)
					printf("\n\t       ");
				printf("%02x ", *(tlv->group_ssid + var_len +
						  sizeof(t_u8) + index));
			}
			var_len += sizeof(t_u8) + index;
			if (tlv->dev_role == 1) {
				for (pi = 0; pi < *(tlv->group_ssid + var_len);
				     pi++) {
					printf("\n\t Peer Mac address(%d) = ",
					       pi);
					print_mac(tlv->group_ssid + var_len +
						  pi);
				}
				num_peers = *(tlv->group_ssid + var_len);
				var_len += (num_peers * ETH_ALEN);
				var_len += sizeof(t_u8);
			}
			if (argc == 2)
				printf("\n\t -----------------------------------------");
...
/**
 *  @brief Show usage information for the wifidirect_cfg_persistent_group_invoke
 * command
 *
 *  $return         N/A
 */
static void print_wifidirect_peristent_group_invoke_usage(void)
{
 ...
 *         sends to the driver
 *
 *  Usage: "Usage : wifidirect_cfg_cmd_persistent_group_invoke [index] |
 *<cancel>"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         SUCCESS or FAILURE
 **/
static void wifidirect_cfg_cmd_persistent_group_invoke(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	tlvbuf_wifidirect_persistent_group *tlv = NULL;
	t_u8 index;
	...
	printf("Persistent group %s successful!\n",
	       (index == WIFIDIRECT_PERSISTENT_RECORD_CANCEL) ? "cancel" :
								"invoke");
done:
	if (buffer)
		free(buffer);
...
/**
 *  @brief Show usage information for the wifidirect presence request parameters
 * command
 *
 *  @return         N/A
 */
static void print_wifidirect_cfg_presence_req_params_usage(void)
{
 ...
 *
 *  Usage: "Usage : wifidirect_cfg_presence_req_params [<type> <duration>
 * <interval>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void wifidirect_cfg_cmd_presence_req_params(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	if (argc < 2) {
		...
		return;
				  ...
				  (is_input_valid(WIFIDIRECT_PRESENCE_REQ_TYPE,
						  1, &argv[2]) != SUCCESS) ||
				  (is_input_valid(WIFIDIRECT_DURATION, 1,
						  &argv[3]) != SUCCESS) ||
				  (is_input_valid(WIFIDIRECT_INTERVAL, 1,
						  &argv[4]) != SUCCESS))) {
		print_wifidirect_cfg_presence_req_params_usage();
		return;
	}
	...
	cmd_buf = (wifidirect_params_config *)buffer;
	tlv = (tlvbuf_wifidirect_presence_req_params
		       *)(buffer + sizeof(wifidirect_params_config));
	...
	tlv->length = sizeof(tlvbuf_wifidirect_presence_req_params) -
		      MRVL_TLV_HEADER_SIZE;
...
/**
 *  @brief Show usage information for the wifidirect extended listen timing
 * parameters command
 *
 *  @return         N/A
 */
static void print_wifidirect_cfg_ext_listen_time_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirect_cfg_cmd_ext_listen_time(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	if (argc < 2) {
		...
		return;
				  ...
				  (is_input_valid(WIFIDIRECT_ATTR_EXTENDED_TIME,
						  1, &argv[2]) != SUCCESS) ||
				  (is_input_valid(WIFIDIRECT_ATTR_EXTENDED_TIME,
						  1, &argv[3]) != SUCCESS))) {
		print_wifidirect_cfg_ext_listen_time_usage();
	...
	cmd_buf = (wifidirect_params_config *)buffer;
	tlv = (tlvbuf_wifidirect_mrvl_ext_listen_time
		       *)(buffer + sizeof(wifidirect_params_config));
	...
	tlv->length = sizeof(tlvbuf_wifidirect_mrvl_ext_listen_time) -
		      MRVL_TLV_HEADER_SIZE;
	if (argc == 2) {
		cmd_buf->action = ACTION_GET;
	} else {
...
/**
 *  @brief Show usage information for the wifidirect provisioing parameters
 * command
 *
 *  @return         N/A
 */
static void print_wifidirect_cfg_provisioning_params_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirect_cfg_cmd_provisioning_params(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
	...
	cmd_buf = (wifidirect_params_config *)buffer;
	tlv = (tlvbuf_wifidirect_provisioning_params
		       *)(buffer + sizeof(wifidirect_params_config));
	...
	tlv->length = sizeof(tlvbuf_wifidirect_provisioning_params) -
		      MRVL_TLV_HEADER_SIZE;
	if (argc == 2) {
		cmd_buf->action = ACTION_GET;
	} else {
 ...
 *  @return         N/A
 */
static void print_wifidirect_cfg_wps_params_usage(void)
{
 ...
 *  @return         None
 */
static void wifidirect_cfg_cmd_wps_params(int argc, char *argv[])
{
	wifidirect_params_config *cmd_buf = NULL;
...
int is_input_valid(valid_inputs cmd, int argc, char *argv[])
{
	...
	case WIFIDIRECT_GROUP_WIFIDIRECT_DEVICE_NAME:
		...
		if ((strlen(argv[0]) > 34)) {
			...
			if ((wifidirect_dev_name[0] != '"') ||
			    (wifidirect_dev_name[strlen(wifidirect_dev_name) -
						 1] != '"')) {
		...
		if ((strlen(argv[0]) > 5) || (strlen(argv[0]) < 4)) {
			...
			if ((country[0] != '"') ||
				...
				for (i = 1; i < (int)strlen(country) - 2; i++) {
					if ((toupper((unsigned char)country[i]) <
					     'A') ||
					    (toupper((unsigned char)country[i]) >
					     'Z')) {
	...
	case WIFIDIRECT_ATTR_EXTENDED_TIME:
		if (argc != 1) {
			...
			if ((ISDIGIT(argv[0]) == 0) ||
			    (atoi(argv[0]) > 65535) || (atoi(argv[0]) < 1)) {
	...
	case WIFIDIRECT_DISC_SERVICEPROTO:
		if (argc != 1) {
			...
			if ((ISDIGIT(argv[0]) == 0) ||
			    ((atoi(argv[0]) >= 4) && (atoi(argv[0]) != 0xFF))) {
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; wifidirect_command[i].cmd; i++) {
		if (strncmp(wifidirect_command[i].cmd, argv[2],
	...
	for (i = 0; wifidisplay_command[i].cmd; i++) {
		if (strncmp(wifidisplay_command[i].cmd, argv[2],
