...
static char *usage[] = {
	...
	"         cloud_keep_alive", "         min_ba_threshold",
	"         11dcfg", "         11dclrtbl", "         addbapara",
	"         addbareject", "         addts", "         amsduaggrctrl",
	"         antcfg",
#ifdef STA_SUPPORT
	"         arpfilter",
#endif
	...
	"         deauth", "         deepsleep", "         delba",
	"         delts",
#if defined(DFS_TESTING_SUPPORT)
	"         dfstesting", "         clear_nop", "         fake_radar",
#endif
	"         dfs53cfg", "         dfs_cac", "         autodfs",
	"         dfs_repeater", "         dscpmap", "         esuppmode",
#ifdef STA_SUPPORT
...
/* #endif */
	...
	"         miracastcfg",
#endif
	"         mgmtframectrl", "         mgmtframetx", "         mpactrl",
	"         netmon",
#ifdef WIFI_DIRECT_SUPPORT
	"         cfg_noa", "         cfg_opp_ps",
#endif
	...
	"         rejectaddbareq", "         scancfg", "         sdcmd52rw",
	"         sdcmd53rw", "         sdioclock",
#ifdef STA_SUPPORT
	"         setuserscan",
#endif
	...
	"         txpowercfg", "         opermodecfg", "         wakeupreason",
	"         warmreset", "         wmmcfg", "         wmmparamcfg",
	"         wpssession",
#ifndef OPCHAN
	"         wwscfg",
#endif
	...
	"         rxpktcoal_cfg",
#endif
	"         get_sensor_temp", "         11k_enable",
	"         neighbor_report", "         channel_switch",
	"         indrstcfg", "         dfs_offload",

	"         per_pkt_cfg", "         dot11_txrx", "         robustcoex",
	"         ctrldeauth", "         dmcs", "         range_ext",
	"         twt_setup", "         twt_teardown", "         rx_abort_cfg",
	"         rx_abort_cfg_ext", "         tx_ampdu_prot_mode",
	"         rate_adapt_cfg", "         cck_desense_cfg",
	"         get_chnrgpwr", "         comparergpwr",
	"         comparetrpc", "         getcfgchanlist", "         lpm",
	"         arb", "         dot11mc_unassoc_ftm_cfg",
...
static int process_11axcmdcfg(int argc, char *argv[])
{
	...
	if (strcmp(argv[3], "obss_pd_offset") == 0) {
		...
		sprintf(argv[3], "%d", MLAN_11AXCMD_CFG_ID_TXOP_RTS);
...
static int process_tdls_setinfo(int argc, char *argv[])
{
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "CapInfo") == 0) {
			...
			memcpy((t_u8 *)(ptr + (akm_index * AKM_SUITE_LEN)),
			...
			memcpy((t_u8 *)(ptr + (pmkid_index * PMKID_LEN)), pmkid,
			       PMKID_LEN);
			pmkid_index++;
...
static int process_hal_phy_cfg(int argc, char *argv[])
{
	...
	if (ret == MLAN_STATUS_SUCCESS) {
		...
		printf("ERR:Could not set the configuration.\n");
	}
...
static int process_mc_aggr_cfg(int argc, char *argv[])
{
	...
	if (argc == 3) {
		...
		mc_cfg->enable_bitmap = atoi(argv[3]);
		mc_cfg->mask_bitmap = atoi(argv[4]);
		if (argc == 6)
...
static int parse_config_file(const char *conf_filename, int *mcast_aggr_grp_num,
			     unsigned int *pkt_len, unsigned int *seq_num,
	...
	while (!feof(fp)) {
		if (fgets(buf, CONFIG_LINE_BUFFER_SIZE, fp)) {
			...
			// read group values
				...
				*seq_num = read_int_from_config_line(buf);
				...
				*expiry_time_conf =
				...
				/* if ampdu number is greater than 1 and in conf
				 * file expiry_time is defined
				 * for single ampdu then replicate the
				 * expiry_time for each ampdu */
				else if ((i == 1) &&
					 (mcast_grp[grp_count].ampduNum > 1)) {
					while (i <
					       (mcast_grp[grp_count].ampduNum)) {
				...
				/* if ampdu number is greater than 1 and in conf
				 * file pkt_len is defined
				 * for single ampdu then replicate the pkt_len
				 * for each ampdu */
				else if ((i == 1) &&
					 (mcast_grp[grp_count].ampduNum > 1)) {
					while (i <
			      ...
			      t_u8 last_retry_grp)
{
	...
	while (count < num_of_packets) {
		...
		/* send retry packets upto 5 limit */
		for (int i = 0; i < pkt_param->pkt_retry; i++) {
...
static int process_mcast_tx(int argc, char *argv[])
{
	...
	if (parse_config_file(argv[3], &mcast_aggr_grp_num, &pkt_len, &seq_num,
		...
		debug_print(mcast_debug_flag,
			    ...
			    seq_num, num_of_iteration, cycle_delay,
			    src_ip_address);
	}
	...
	for (grpid = 0; grpid < mcast_aggr_grp_num; grpid++) {
		...
		for (int pktid = 0; pktid < mcast_grp[grpid].ampduNum;
			...
			if (mcast_grp[grpid].pkt_data[pktid].pkt_retry) {
				...
				if (first_retry_pkt) {
					mcast_grp[grpid].first_retry_pkt =
						pktid;
					mcast_grp[grpid].last_retry_pkt = pktid;
					first_retry_pkt = 0;
				} else {
					mcast_grp[grpid].last_retry_pkt = pktid;
				}
	...
	while (num_of_iteration > cycle++) {
		...
		for (grpid = 0; grpid < mcast_aggr_grp_num; grpid++) {
			...
			if (mask_grp) {
				 ...
				 * tsf expiry time
				 * pass lower 32bits of absolute packet expiry
				 * TSF */
				txCtrlPtr->tsf =
					(tsf_pkt_expiry +
					 (pkt_param->expiry_time * 1000)) &
			...
			while (count < no_of_packets) {
				...
				if (mcast_grp[grpid].retry)
					pkt_param->retry_seq_num = pkt_seq_num;
		...
		/* send rety packets */
		for (grpid = 0; grpid < mcast_aggr_grp_num; grpid++) {
			/* send retry packets now for a cycle after new packets
			 */
			if (mcast_grp[grpid].retry) {
				if (send_retry_packets(
					    &mcast_grp[grpid], grpid, sockFd,
					    buffer, &tsf_pkt_expiry,
					    (grpid == last_retry_grpnum)) ==
				    MLAN_STATUS_FAILURE) {
					ret = MLAN_STATUS_FAILURE;
					goto done;
				}
...
static int process_ch_load(int argc, char *argv[])
{
	...
	printf("action=%d ch_load=%d\n", cl_cfg->action, cl_cfg->ch_load_param);
...
#ifdef RX_PACKET_COALESCE
static void print_rx_packet_coalesc_help()
{
...
static int process_drcs_time_slicing_cfg(int argc, char *argv[])
{
	...
	if (argc == 3) { /* GET operation */
		...
		printf("Channel index%d: chantime=%d switchtime=%d undozetime=%d mode=%d\n",
		       ...
		       drcs_cfg->switchtime, drcs_cfg->undozetime,
		       drcs_cfg->mode);
		drcs_cfg++;
		printf("Channel index%d: chantime=%d switchtime=%d undozetime=%d mode=%d\n",
		       (drcs_cfg->chan_idx - 1), drcs_cfg->chantime,
...
static int process_get_sensor_temp(int argc, char *argv[])
{
	...
	printf("SOC temperature is %u.%u C \n", temp / 10, temp % 10);
