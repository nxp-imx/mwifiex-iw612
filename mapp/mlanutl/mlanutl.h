...
struct eth_priv_hs_cfg {
	...
	t_u32 gpio;
	/** Gap in milliseconds or or 0xff for special setting when GPIO is used
	 * to wakeup host */
	t_u32 gap;
...
enum { MLAN_SCAN_MODE_UNCHANGED = 0,
       MLAN_SCAN_MODE_BSS,
       MLAN_SCAN_MODE_IBSS,
       MLAN_SCAN_MODE_ANY };

/** Enumeration for scan type */
enum { MLAN_SCAN_TYPE_UNCHANGED = 0,
...
typedef struct _mcast_tx_pkt_data {
	...
	/** pkt expiry time */
	t_u32 expiry_time;
	/** retry packet flag, 0 for no retry, 5 for max retry */
	t_u8 pkt_retry;
	/** retry seq number */
	t_u32 retry_seq_num;
} mcast_tx_pkt_data;
...
typedef struct _mcast_tx_grp_data {
	...
	/** retry packet flag, 0 for no retry, 1 for retry */
	t_u8 retry;
	/** Tx aggregated mpdu */
	t_u32 ampduNum;
	/** mc_pkt_flags */
